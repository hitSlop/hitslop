...
@Test(arguments: ["preview", "png", "pdf", "icon", "error", "cancel", "timeout"])
@MainActor func disposableCaptureClosesRetainedSessionBeforeRemovingSQLite(_ scenario: String) async throws {
  let root = try rendererPackage()
  defer { try? FileManager.default.removeItem(at: root.deletingLastPathComponent()) }
  try addDocumentFixture(root)
  if scenario == "timeout" {
    let app = root.appendingPathComponent("app.html")
    let html = try String(contentsOf: app, encoding: .utf8)
      .replacingOccurrences(of: "window.slop.ready()", with: "void 0")
    try Data(html.utf8).write(to: app)
  }
  let snapshot = try await SlopRenderSnapshot.prepare(packageURL: root)
  defer { snapshot.remove() }
  var heldSession: SlopRuntimeSession?
  let entered = AsyncStream<Void>.makeStream()
  let job = Task { @MainActor in
    try await SlopRenderer.withCaptureSession(snapshot: snapshot, renderTargetsEnabled: true,
      readinessTimeout: scenario == "timeout" ? .milliseconds(100) : .seconds(15)) { session in
      heldSession = session
      // Even an explicit removal request cannot delete an actively used snapshot.
      snapshot.remove()
      #expect(FileManager.default.fileExists(atPath: snapshot.url.path))
      entered.continuation.yield(())
      switch scenario {
      case "preview": #expect(try await SlopRenderer.previewPNGData(session: session).count > 0)
      case "png": #expect(try await SlopRenderer.exportPNGData(session: session).count > 0)
      case "pdf": #expect(try await SlopRenderer.exportPDFData(session: session).count > 0)
      case "icon": #expect(try await SlopRenderer.targetPNGData(session: session, target: .icon) != nil)
      case "error": throw SlopPackageError.invalid("injected capture failure")
      case "cancel": try await Task.sleep(for: .seconds(30))
      default: Issue.record("Timed-out session must not capture")
      }
    }
  }
  if scenario == "cancel" {
  ...
  switch result {
  case .success: #expect(!["error", "cancel", "timeout"].contains(scenario))
  case .failure: #expect(["error", "cancel", "timeout"].contains(scenario))
  }
  if let heldSession {
    let document = try #require(heldSession.document)
    #expect(await document.databaseIsClosed)
    try await heldSession.closeAndWait()
    #expect(await document.databaseIsClosed)
  }
  snapshot.remove()
  #expect(!FileManager.default.fileExists(atPath: snapshot.url.path))
...
@Test func failedCaptureTeardownPreservesSnapshotEvenAfterRemovalRequest() throws {
  let root = try rendererPackage()
  defer { try? FileManager.default.removeItem(at: root.deletingLastPathComponent()) }
  let snapshot = try SlopRenderSnapshot(packageURL: root)
...
@Test @MainActor func staticCaptureHidesControlsRestoresTheDocumentAndUsesTheRequestedGeometry()
  async throws
{
  let root = try rendererPackage()
  defer { try? FileManager.default.removeItem(at: root.deletingLastPathComponent()) }
  ...
  let previewImage = try #require(
    CGImageSourceCreateWithData(preview as CFData, nil).flatMap {
  ...
  let iconImage = try #require(
    CGImageSourceCreateWithData(icon as CFData, nil).flatMap {
      CGImageSourceCreateImageAtIndex($0, 0, nil)
    })
  ...
  let exportImage = try #require(
    CGImageSourceCreateWithData(exported as CFData, nil).flatMap {
      CGImageSourceCreateImageAtIndex($0, 0, nil)
    })
  ...
  let state =
    try await session.webView.evaluateJavaScript(
      "({capture:document.documentElement.getAttribute('data-slop-capture'),display:getComputedStyle(document.querySelector('button')).display,target:getComputedStyle(document.querySelector('[data-slop-render=icon]')).display,background:getComputedStyle(document.body).backgroundColor})"
    ) as? [String: Any]
...
@Test @MainActor func iconTargetLargerThanTheWindowStillRendersAt512AndRestoresTheViewport()
  async throws
{
...
@Test @MainActor func documentAssetsRenderInAHiddenSessionWithoutTouchingTheEditor() async throws {
  ...
  let window = NSWindow(
    contentRect: visibleSession.webView.frame, styleMask: [.borderless], backing: .buffered,
    defer: false)
  ...
  let state =
    try await visibleSession.webView.evaluateJavaScript(
      "({capture:document.documentElement.getAttribute('data-slop-capture'),active:document.activeElement?.tagName,mutations:window.__captureMutations,targets:document.querySelectorAll('[data-slop-render]').length,background:getComputedStyle(document.body).backgroundColor})"
    ) as? [String: Any]
  #expect(state?["capture"] is NSNull)
...
private func rendererPackage(
  includeTargets: Bool = true, duplicateIcon: Bool = false, targetSize: Int = 200
) throws -> URL {
  let parent = FileManager.default.temporaryDirectory.appendingPathComponent(
    "hitslop-renderer-\(UUID().uuidString)", isDirectory: true)
  ...
  let manifest =
    #"{"$schema":"https://api.hitslop.com/schemas/v1/manifest.schema.json","author":{"name":"Fixture Author","url":"https://example.com"},"slug":"renderer-test","title":"Renderer Test","description":"Static capture fixture.","categories":["developer-tools"],"presentation":{"width":320,"height":240}}"#
...
private func rgba(_ image: CGImage, x: Int, y: Int) throws -> (
...
@Test @MainActor func capturePreparationFailureRestoresSelectionAndNestedScroll() async throws {
  ...
  let state = try #require(
    try await session.webView.evaluateJavaScript(
      "({capture:document.documentElement.getAttribute('data-slop-capture'),active:document.activeElement.id,start:document.activeElement.selectionStart,end:document.activeElement.selectionEnd,scroll:document.querySelector('#scroll').scrollTop})"
    ) as? [String: Any])
  #expect(state["capture"] is NSNull)
...
@Test @MainActor func dedicatedExportWaitsForAsyncContentAndPreservesCurrentView() async throws {
  ...
  let image = try #require(
    CGImageSourceCreateWithData(imageData as CFData, nil).flatMap {
      CGImageSourceCreateImageAtIndex($0, 0, nil)
    })
  ...
  #expect(
    try await session.webView.evaluateJavaScript("document.querySelector('button').textContent")
      as? String == "Control")
}
...
@Test @MainActor func longVectorExportIsNotSubjectToRasterLimits() async throws {
  ...
  try await session.waitUntilReady()
  _ = try await session.webView.evaluateJavaScript(
    "document.body.style.minHeight='18000px';document.querySelector('.tail').style.top='17900px'")
  await #expect(throws: SlopPackageError.self) {
    _ = try await SlopRenderer.exportPNGData(session: session)
  }
  let pdf = try #require(PDFDocument(data: try await SlopRenderer.exportPDFData(session: session)))
...
@Test @MainActor func iconFailureDoesNotDiscardPreviewAndRenderingDoesNotInitializeOriginalStores()
  async throws
{
  ...
  html = html.replacingOccurrences(
    of: "window.slop.ready()", with: "window.slop.document.open().then(()=>window.slop.ready())")
  try Data(html.utf8).write(to: app)
  let assets = try await SlopRenderer.documentAssetsPNGData(packageURL: root)
...
@Test @MainActor func overlappingExportsSerializeAndRestoreTheEditor() async throws {
  ...
  #expect(
    try await session.webView.evaluateJavaScript(
      "document.documentElement.getAttribute('data-slop-capture')") is NSNull)
}
...
@Test @MainActor func backgroundCaptureKeepsExistingJSONMediaAndThemeUnchanged() async throws {
  ...
  session.close()
  try Data(":root{--slop-ink:#123456}".utf8).write(
    to: root.appendingPathComponent("stores/theme.css"))
  let paths = [
    "state/document.sqlite", "stores/data.json", "stores/media/\(photoHash)", "stores/theme.css",
  ]
  let originals = try paths.map { try Data(contentsOf: root.appendingPathComponent($0)) }
  let app = root.appendingPathComponent("app.html")
  let html = try String(contentsOf: app, encoding: .utf8).replacingOccurrences(
    of: "window.slop.ready()",
    with: #"""
  ...
  for (index, path) in paths.enumerated() {
    #expect(try Data(contentsOf: root.appendingPathComponent(path)) == originals[index])
  }
}
...
@Test @MainActor func invalidatedCloseJobCannotReplacePreview() async throws {
  let root = try rendererPackage()
  defer { try? FileManager.default.removeItem(at: root.deletingLastPathComponent()) }
  SlopDocumentAssetRefreshQueue.schedule(
    snapshot: try SlopRenderSnapshot(packageURL: root), presentedURL: root)
  ...
  let app = root.appendingPathComponent("app.html")
  let stalled = try String(contentsOf: app, encoding: .utf8).replacingOccurrences(
    of: "window.slop.ready()", with: "void 0")
...
@MainActor func compiledChecklistExportsSelectedViewLongContentAndDynamicIcon() async throws {
  ...
  let tasks: [[String: Any]] =
    (0..<120).map {
      [
        "id": "task-\($0)", "text": "Task \($0) with a second line\nand extra detail",
        "done": $0 == 0, "archived": false,
      ]
    } + [["id": "filed", "text": "Only filed item", "done": true, "archived": true]]
  let owner = try #require(
    try SlopCommandDocument.open(package: SlopPackage(rootURL: snapshot.url)))
  let open = try await owner.openGuest()
  let data = String(
    decoding: try JSONSerialization.data(withJSONObject: [
      "title": "A long checklist", "tasks": tasks,
    ]), as: UTF8.self)
  let request: String = try owner.engine.call(
    "replace", [open.snapshot.json, data, open.lease.json, "seed"])
  _ = try await owner.apply(request)
  ...
  _ = try await session.webView.callAsyncJavaScript(
    "document.querySelector('[role=tab][data-value=filed]')?.click(); const tabs=[...document.querySelectorAll('[role=tab]')]; tabs.find(e=>e.textContent.includes('Filed')).click(); await new Promise(r=>setTimeout(r,0)); return true",
    arguments: [:], in: nil, contentWorld: .page)
  let filed = try #require(
    PDFDocument(data: try await SlopRenderer.exportPDFData(session: session)))
  ...
  if let output = ProcessInfo.processInfo.environment["HITSLOP_PILOT_OUTPUT"] {
    ...
    try #require(long.dataRepresentation()).write(
      to: directory.appendingPathComponent("long-checklist.pdf"))
    try #require(filed.dataRepresentation()).write(
      to: directory.appendingPathComponent("filed-checklist.pdf"))
...
private func addDocumentFixture(_ root: URL) throws {
  let schema =
    #"{"type":"object","properties":{"count":{"type":"integer"}},"required":["count"],"additionalProperties":true,"x-hitslop":{"version":1,"container":"map"}}"#
  ...
  return try JSONSerialization.data(withJSONObject: [
    ...
    "properties": [
      ...
      "$slop": [
        ...
        "properties": [
          "format": ["const": 2], "documentId": ["type": "string", "minLength": 1],
