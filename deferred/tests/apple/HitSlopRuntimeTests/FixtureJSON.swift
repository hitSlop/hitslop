...
// Test fixture construction/inspection only. Never used to transport production data.
public enum FixtureJSON: Codable, Equatable, Sendable {
  case null
  case bool(Bool)
  case number(Double)
  case string(String)
  ...
  public static func == (lhs: Self, rhs: Self) -> Bool {
    ...
    case (.object(let a), .object(let b)):
      ...
      return a.allSatisfy { key, value in
        guard let index = b.index(forKey: key), key.utf8.elementsEqual(b[index].key.utf8) else {
          return false
        }
  ...
  public init(from decoder: any Decoder) throws {
    ...
    if c.decodeNil() {
      ...
      self = .string(v)
    } else if let v = try? c.decode([FixtureJSON].self) {
      self = .array(v)
    } else {
      self = .object(try c.decode([String: FixtureJSON].self))
    }
    switch self {
    case .array, .object:
      guard decoder.codingPath.count < SlopJSONLimits.framedDepth else {
  ...
  public func encode(to encoder: any Encoder) throws {
    ...
    switch self {
    case .array, .object: isContainer = true
  ...
  public init(
    data: Data, maximumBytes: Int = SlopFile.maximumBytes,
    maximumDepth: Int = SlopJSONLimits.framedDepth
  ) throws {
    try SlopJSONLimits.check(data, maximumBytes: maximumBytes, maximumDepth: maximumDepth)
    self = try Self.foundation(
  ...
  private static func foundation(_ value: Any) throws -> Self {
    ...
    if let object = value as? [String: Any] {
      return .object(try object.mapValues(Self.foundation))
    }
    throw SlopDocumentError("Expected plain JSON")
  }
  /// Conservative preflight; encoded() enforces the exact UTF-8 byte budget.
  public func validateBounds(
    maximumBytes: Int = SlopJSONLimits.documentBytes,
    maximumDepth: Int = SlopJSONLimits.maximumDepth
  ) throws {
    var bytes = 0
    func walk(_ value: Self, depth: Int) throws {
      ...
      case .object(let value):
        guard depth < maximumDepth else {
          throw SlopLimitError("Document exceeds \(maximumDepth) nesting levels")
        }
  ...
  public func encoded(
    maximumBytes: Int = SlopFile.maximumBytes, maximumDepth: Int = SlopJSONLimits.framedDepth
  ) throws -> Data {
  ...
  public var object: [String: FixtureJSON] { if case .object(let v) = self { v } else { [:] } }
  public var array: [FixtureJSON] { if case .array(let v) = self { v } else { [] } }
  ...
  public subscript(_ key: String) -> FixtureJSON {
    get { object[key] ?? .null }
    set {
      var v = object
      v[key] = newValue
