import { safeArchivePath } from "./package-path.js";
...
export function inspectZip(
  bytes: Uint8Array,
  validateEntry: (name: string) => void = () => {},
): void {
  ...
  if (entries === 0 || entries > MAX_ENTRIES)
    throw new Error(`Artifact must contain 1–${MAX_ENTRIES} entries`);
  ...
  for (let index = 0; index < entries; index += 1) {
    if (offset + 46 > eocd || view.getUint32(offset, true) !== 0x02014b50)
      throw new Error("Artifact has an invalid ZIP entry");
    ...
    if (method !== 0 && method !== 8)
      throw new Error("Artifact contains an unsupported compression method");
    ...
    const payload = localOffset + 30 + localNameLength + localExtraLength;
    if (
      payload + compressed > centralOffset ||
      ...
      throw new Error("Artifact has inconsistent ZIP headers");
    }
    offset = end;
  }
  if (offset !== centralOffset + centralBytes)
    throw new Error("Artifact has an invalid ZIP directory size");
}
