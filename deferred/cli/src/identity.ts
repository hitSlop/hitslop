...
const service = "app.hitslop.cli";
const account = "publisher";
const fallback = join(homedir(), ".hitslop", "identity.json");
...
async function keychainWrite(value: string): Promise<boolean> {
...
async function readStored(): Promise<Identity | null> {
  const value = (await keychainRead()) ?? (await readFile(fallback, "utf8").catch(() => null));
...
async function storeIdentity(identity: Identity): Promise<void> {
  ...
  if (!(await keychainWrite(json))) {
    await mkdir(join(homedir(), ".hitslop"), { recursive: true });
    await writeFile(fallback, json, { mode: 0o600 });
...
export async function importIdentity(
  ...
  try {
    identity = JSON.parse(
      Buffer.concat([decipher.update(decode(payload.ciphertext)), decipher.final()]).toString(
        "utf8",
      ),
    ) as Identity;
  } catch {
...
export const sign = async (message: Uint8Array, identity: Identity): Promise<string> =>
  encode(await signAsync(message, decode(identity.privateKey)));
