...
@MainActor @Observable final class DocumentSharing {
  ...
  init(
    controller: SlopDocumentWindowController, account: StoreOf<AccountFeature>, api: SlopCloudAPI
  ) {
  ...
  func stop() {
    ...
    disconnect()
    if let panel {
      controller?.window?.removeChildWindow(panel)
      panel.close()
    }
    panel = nil
  }
  ...
  func show() {
    ...
    if panel == nil {
      let panel = NSPanel(
        contentRect: NSRect(x: 0, y: 0, width: 360, height: 420), styleMask: [.titled, .closable],
  ...
  func startSharing() async {
    ...
    guard account.user != nil else {
      error = "Sign in to share this document."
      return
    }
    disconnect()
    let attempt = generation
    ...
    do {
      ...
      let needed = try await SlopMediaSync.hashes(
        in: document.frame().data, using: document.engine)
      try await SlopMediaSync.uploadMissing(
        package: controller.session.package, api: api, needed: needed)
      let packed = try await document.sharingBundle(
        SlopArchive.packSharedApp(controller.session.package.rootURL))
      let shared = try await api.createDocument(
        id: seed.documentId, title: controller.session.package.manifest.title,
        slug: controller.session.package.manifest.slug, schema: seed.schemaHash, package: packed,
        seed: seed
      )
  ...
  func updateInvitation(enabled: Bool) async {
    ...
    defer { busy = false }
    do {
      self.room = try await api.updateInvitation(documentId: room.documentId, enabled: enabled)
    } catch { self.error = error.localizedDescription }
  }
  func removeMember(_ id: String) async {
    ...
    busy = true
    defer { busy = false }
    do { self.room = try await api.removeMember(documentId: room.documentId, memberId: id) } catch {
  ...
  private func attach(shared: SlopSharedDocument, document: SlopCommandDocument) {
    ...
    let next = SlopRoomSession(
      origin: api.origin, documentId: shared.documentId, schema: shared.schema, document: document,
      credentials: { try await api.session(documentId: shared.documentId) },
      media: transfer.transport)
    ...
    next.onStatus = { [weak self] status in
      switch status {
      case .live:
        self?.status = "Live on Cloudflare"
        self?.error = nil
    ...
    media = Task { [weak self] in
      ...
      do {
        ...
        for await frame in try await document.events() {
          ...
          let needed = try await SlopMediaSync.hashes(in: frame.data, using: document.engine)
  ...
  private func connectIfShared(attempt: UUID) async -> Bool {
    ...
    guard await document.mode != .local else {
    ...
    do {
      ...
      if await document.mode == .promoting, let controller, let seed = await document.sharingSeed()
      {
        let packed = try await document.sharingBundle(
          SlopArchive.packSharedApp(controller.session.package.rootURL))
        shared = try await api.createDocument(
          id: identity.documentId, title: controller.session.package.manifest.title,
          slug: controller.session.package.manifest.slug, schema: identity.schema, package: packed,
          seed: seed)
      } else {
        shared = try await api.document(identity.documentId)
      }
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: 14) {
      ...
      if !model.supportsCollaboration {
        ...
        Text("Sign in from Account to live-share this document and its media.").foregroundStyle(
          .secondary)
      } else if let room = model.room {
        Text(room.title).font(.body.weight(.medium))
        Text(
          "\(room.members.count) member\(room.members.count == 1 ? "" : "s") · \(model.peers.count) here now"
        )
        ...
        ForEach(room.members, id: \.id) { member in
          HStack {
            ...
            if room.owner == model.account.user?.id, member.id != room.owner {
              Button("Remove", role: .destructive) { Task { await model.removeMember(member.id) } }
                .disabled(model.busy)
            }
          }
        }
        ...
        if room.owner == model.account.user?.id {
          Button(room.invitationsEnabled ? "Replace invite link" : "Enable invitations") {
            Task { await model.updateInvitation(enabled: true) }
          }.disabled(model.busy)
          if room.invitationsEnabled {
            Button("Disable invitations") { Task { await model.updateInvitation(enabled: false) } }
              .disabled(model.busy)
          }
        }
      } else {
        Text(
          "Upload this app and document to create a live share link. Friends can open it in hitSlop, including apps that aren’t in the catalog."
