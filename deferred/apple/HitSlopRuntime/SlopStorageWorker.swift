...
final class SlopStorageWorker: @unchecked Sendable {
    ...
    await withCheckedContinuation { continuation in
      queue.async {
        guard !self.closed else {
          continuation.resume(returning: SlopRevisions(media: nil, theme: nil))
          return
        }
        continuation.resume(
          returning: SlopRevisions(
            media: try? self.media.directoryRevision(), theme: try? self.theme.revision()))
      }
    }
  }
  ...
  private func field<T>(_ name: String, in body: [String: Any], as type: T.Type = T.self) throws
    -> T
  {
    guard let raw = body[name], let value = raw as? T else {
      throw SlopBridgeFailure(.invalidRequest, "Missing or invalid \(name)")
    }
    return value
  }
  private func handle(_ body: [String: Any]) throws -> SlopStorageResult {
    ...
    case .mediaAdd:
      ...
      return .init(
        value: ["sha256": reference.sha256, "bytes": reference.bytes, "mime": reference.mime],
        revision: reference.sha256, sha256: reference.sha256)
