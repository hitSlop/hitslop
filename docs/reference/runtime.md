...
A `.slop` combines immutable authored app code with a structured local document. Each WebView owns one live Loro replica. The host supplies the document SDK and pinned JS/WASM engine for its selected runtime contract. Svelte observes immutable snapshots and writes typed operations. Swift persists opaque bytes; it does not interpret application fields.
...
Autosave runs after 200 ms. Flush commits text drafts, persists updates, and checkpoints at 256 updates or 4 MiB. Native load/write limits are 4,096 rows and 32 MiB aggregate checkpoint/update bytes. The engine checkpoints before appending beyond those limits. Oversized snapshots fail visibly while retaining pending edits. Oversized existing packages are refused intact; compact cannot promise recovery of unloadable packages.
...
Document windows remain hidden until runtime readiness, fonts, and a bounded paint opportunity. Opens taking more than 300 ms display cancellable native progress. Ready documents appear directly, without a preview swap or fade. Closing cancels presentation. Startup failures reveal native error UI. Quick Look artwork serves Finder and catalog display, not an opening placeholder.

Debug `HITSLOP_STARTUP_TIMINGS=1` logs elapsed preparation, readiness, and presentation without document values or paths. `HITSLOP_STARTUP_BENCH=1 bun run swift:test --filter documentStartupTimings` measures sequential opens, distinguishing first and subsequent samples. Caches and machine activity affect these diagnostics; they are not CI thresholds.
...
CSP permits local scripts/WASM, local connections, inline styles, and local/data images and fonts. Arbitrary remote fetches are blocked. Native navigation cancels external navigation; explicit HTTP(S) links open in the system browser. Camera/microphone grants are not part of v1.
