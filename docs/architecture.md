...
No JavaScriptCore engine, JSON projection, external JSON editing, proposal recovery, or room authority remains in the active graph. The existing macOS client, TCA, local catalog, Firebase, OpenAPI generation, and native CLI remain active. Hosted catalog, public publishing, document sharing, and account UI are deferred. Firebase startup remains enabled. The old `slop` s-expression skill is unrelated to this package format.
