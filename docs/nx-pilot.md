...
Three attempts use the same SHA and fresh GitHub runners. Previously populated entries with identical declared build inputs may be reused; report actual execution counts rather than assuming the first attempt is cold. Linux compares direct compilation with Nx and verifies identical portable bytes. macOS compares the unchanged production template builder, direct compilation/rendering with the same concurrency as Nx, and Nx remote/local reuse. Its portable bytes must also match Linux's results. The uncached full macOS baseline runs once; the subsequent attempts measure fresh-runner cache reuse and edits without repeating that expensive control. All timed paths include package/runtime verification; complete macOS paths include inventory assembly.

The production builder uses a dedicated Actions cache namespace; no benchmark touches the production template cache. Nx local artifact caches and their separate SQLite workspace indexes are deleted before remote measurements and never uploaded as Actions caches. Each attempt makes a new single-slop edit, so edit measurements cannot reuse the previous attempt's changed package. The existing builder's restore/save durations and shared native helper/cache setup are reported separately from build commands. The benchmark deliberately builds the corpus several ways; its total duration is not the proposed production build time.
...
A second diagnostic attempt ([36165181217](https://github.com/hitSlop/hitslop/actions/runs/36165181217)) passed all full-corpus builds and cross-platform byte comparisons, then exposed incomplete benchmark cleanup: deleting `.nx/cache` without `.nx/workspace-data` left stale local cache index entries. Fresh-runner simulation now clears both checkout-local directories. That failed edit timing is excluded from adoption comparisons. Existing valid cloud and Actions entries remain usable because the build inputs did not change.
