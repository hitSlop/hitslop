...
| Imported state readable by historical runtimes ([compatibility tests](../packages/cli/tests/compatibility.test.ts)) | Independent field/reference expectations followed by sealed revision-1 readers in isolated processes, for both updates and checkpoints | 0 | Existing corpus covers old authored commands, not the new importer | EXTEND without changing historical fixtures or release records |
