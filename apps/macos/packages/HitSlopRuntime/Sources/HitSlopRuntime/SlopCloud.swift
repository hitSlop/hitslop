...
enum SlopWorkingCopy {
    ...
    static func pushStores(from working: URL, to presented: URL) throws {
        ...
        try coordinate(writing: presented) { destination in
            for (id, store) in package.manifest.stores {
                let source = try package.store(id: id, kind: store.kind)
                let target = destination.appendingPathComponent(SlopPackage.storePath(id: id, kind: store.kind))
    ...
    private static func snapshotSQLite(in root: URL) throws {
        ...
        for (id, store) in package.manifest.stores where store.kind == .sqlite {
    ...
    private static func coordinate(writing url: URL, _ body: (URL) throws -> Void) throws {
        var coordinatorError: NSError?
        var bodyError: Error?
        NSFileCoordinator().coordinate(writingItemAt: url, options: .forMerging, error: &coordinatorError) { newURL in
            do { try body(newURL) } catch { bodyError = error }
        }
        if let coordinatorError { throw coordinatorError }
        if let bodyError { throw bodyError }
...
@MainActor public final class SlopOpenedDocument {
    ...
    public func scheduleFlush() {
        ...
        flushWork?.cancel()
        let work = DispatchWorkItem { [weak self] in try? self?.flush() }
        flushWork = work
