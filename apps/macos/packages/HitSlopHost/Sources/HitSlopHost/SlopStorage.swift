...
final class SlopJSONStore {
    let url: URL
    let maxBytes: Int
    init(url: URL, maxBytes: Int) { self.url = url; self.maxBytes = maxBytes }

    func read() throws -> (value: Any, revision: String) {
        let data = try Data(contentsOf: url)
        guard data.count <= maxBytes else { throw SlopPackageError.invalid("JSON store exceeds maxBytes") }
    ...
    func write(_ value: Any, expectedRevision: String?) throws -> String {
        ...
        guard JSONSerialization.isValidJSONObject(value) else { throw SlopPackageError.invalid("JSON store value must be an object or array") }
    ...
    func revision() throws -> String { Self.revision(try Data(contentsOf: url)) }
...
final class SlopDatabase {
    ...
    func transaction(_ statements: [(String, [Any])], validate: () throws -> Void) throws -> Int {
