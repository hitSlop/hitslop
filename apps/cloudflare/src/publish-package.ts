...
export function validatePackageMetadata(files: Record<string, Uint8Array>): void {
  ...
  if (schema) {
    try {
      ...
      checkDataSchema(value);
      validateDocument(
        applicationSchema(value),
        JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(files["assets/initial.json"])),
      );
    } catch (error) {
      throw new Error(
        `Artifact data schema must be valid UTF-8 JSON: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  if (guide) {
    if (guide.byteLength > maxDocumentGuideBytes)
      throw new Error(`${documentGuidePath} exceeds 32 KiB`);
    try {
      new TextDecoder("utf-8", { fatal: true }).decode(guide);
    } catch {
...
export function validateStaticImages(files: Record<string, Uint8Array>): {
  preview: Uint8Array;
  icon: Uint8Array;
} {
  ...
  if (!preview || !icon)
    throw new Error("Artifact must contain QuickLook/Preview.png and QuickLook/Icon.png");
