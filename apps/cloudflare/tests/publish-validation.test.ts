  ...
  expect(() =>
    inspectZip(
      zipSync({
        ...
        "assets/icon.txt": bytes("ok"),
      }),
    ),
  ).not.toThrow();
  expect(() => inspectZip(zipSync({ "../secret": bytes("no") }))).toThrow("Unsafe archive entry");
  expect(() => inspectZip(zipSync({ "source/App.svelte": bytes("no") }))).toThrow(
    "Template packages cannot contain",
  );
