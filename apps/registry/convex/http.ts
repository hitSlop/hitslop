...
http.route({ path: "/internal/publish", method: "POST", handler: httpAction(async (ctx, request) => {
  ...
  for (const key of ["requestId", "publisherKeyId", "publicKey", "displayName", "artifactKey", "artifactSha256", "previewKey", "previewSha256"]) {
    if (typeof value[key] !== "string") return Response.json({ error: `Invalid ${key}` }, { status: 400 });
  }
  if (typeof value.artifactBytes !== "number" || typeof value.previewBytes !== "number" || typeof value.manifest !== "object") {
