...
Pass it as the `icon` snippet on `<Slop>` and use the same document store as
the editor. Do not open a second store inside the icon.
...
<Slop {document}>
  ...
  {#snippet icon()}
    <Icon count={document.data.count} />
  {/snippet}
  {#snippet exportView()}
    <Export count={document.data.count} />
  {/snippet}
</Slop>
```

The snippet mounts only in the icon renderer and provides a transparent
...
```svelte title="src/App.svelte"
{#snippet exportView()}
  <Export title={document.data.title} items={document.data.tasks} />
{/snippet}
```
...
`createDocument` already reports host readiness, including a failed first open.
Test long, empty, and wrapped content before publishing.
