...
@main struct NativeCLI: AsyncParsableCommand {
    static let configuration = CommandConfiguration(commandName: "hitslop-native", abstract: "Native rendering companion for @hitslop/cli.", subcommands: [Screenshot.self, Export.self, OpenDev.self, Create.self, Open.self])
}
...
struct Create: AsyncParsableCommand {
    ...
    @MainActor func run() async throws {
        ...
        if let source {
            try DocumentFactory(catalogURL: URL(string: "https://api.hitslop.com")!).create(fromLocalPackage: source, at: output, requirePreview: false)
...
struct Open: AsyncParsableCommand {
    ...
    @MainActor func run() async throws {
        ...
        _ = try await NSWorkspace.shared.open([package], withApplicationAt: app, configuration: configuration)
        print(package.path)
    }
}
