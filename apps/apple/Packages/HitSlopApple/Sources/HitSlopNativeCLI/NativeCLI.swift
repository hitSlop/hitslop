...
@main struct NativeCLI: AsyncParsableCommand {
    static let configuration = CommandConfiguration(commandName: "hitslop-native", abstract: "Native rendering companion for @hitslop/cli.", subcommands: [Screenshot.self, Export.self, OpenDev.self, Inspect.self, Apply.self, Create.self, Open.self])
}
...
struct Inspect: AsyncParsableCommand {
    ...
    @MainActor func run() async throws {
        ...
        print(String(data: try encoder.encode(frame.data), encoding: .utf8)!)
    }
}
...
struct Create: AsyncParsableCommand {
    ...
    @MainActor func run() async throws {
        let output = self.output.pathExtension.lowercased() == "slop" ? self.output : self.output.appendingPathExtension("slop")
        ...
        guard (source == nil) != (catalogEntry == nil) else { throw ValidationError("Provide --from or --catalog-entry") }
        guard !DocumentFactory.isManagedTemplatePackage(output), !DocumentFactory.isManagedTemplatePackage(output, templatesRoot: templatesRoot) else { throw ValidationError("A document cannot be created in the template cache") }
        try FileManager.default.createDirectory(at: output.deletingLastPathComponent(), withIntermediateDirectories: true)
        if let source {
            try DocumentFactory(catalogURL: URL(string: "https://api.hitslop.com")!).create(fromLocalPackage: source, at: output)
        } else if let catalogEntry {
            let entry = try JSONDecoder().decode(Entry.self, from: Data(contentsOf: catalogEntry))
            guard entry.publisherKeyID.range(of: "^[a-zA-Z0-9_-]{16,64}$", options: .regularExpression) != nil,
            ...
            _ = try await DocumentFactory(catalogURL: entry.catalogURL, templatesRoot: templatesRoot).create(from: SlopRemoteTemplate(
            ...
            try? await SlopCloudAPI(origin: entry.catalogURL).recordCreation(templateId: "\(entry.publisherKeyID)_\(entry.slug)")
        }
        SlopPreviewWriter.installExistingPreview(for: output)
        print(output.path)
    }
}
...
struct Open: AsyncParsableCommand {
    ...
    @MainActor func run() async throws {
        ...
        guard !DocumentFactory.isManagedTemplatePackage(package) else { throw ValidationError("Create a writable copy of this template first") }
        ...
        try await NSWorkspace.shared.open([package], withApplicationAt: app, configuration: configuration)
        print(package.path)
    }
}
