...
@MainActor
private final class CommandBenchmarkBridge: NSObject, WKScriptMessageHandlerWithReply,
  WKNavigationDelegate
{
  let storage: SlopCommandStorage
  let schema: FixtureSchema
  var loaded: CheckedContinuation<Void, Never>?
  init(storage: SlopCommandStorage, schema: FixtureSchema) {
  ...
  func userContentController(
    ...
    do {
      ...
      let request = try FixtureJSON(data: Data((message.body as! String).utf8))
...
struct CommandPerformanceTests {
  ...
  @Test @MainActor func webKitCheckpoint() async throws {
    ...
    let schemaJSON = try FixtureJSON(
      data: Data(
    ...
    for rows in [1_000, 5_000, 10_000] {
      ...
      configuration.userContentController.addScriptMessageHandler(
        bridge, contentWorld: .page, name: "commands")
      let web = WKWebView(
      ...
      for renderRows in [false, true] {
        let output = try await web.callAsyncJavaScript(
          ...
          """#,
          arguments: [
            "opening": opening.json, "renderRows": renderRows,
          ], in: nil, contentWorld: .page)
        print(
          "[command checkpoint] rows=\(rows) renderRows=\(renderRows) samples=\(output ?? "nil")")
      }
      configuration.userContentController.removeScriptMessageHandler(
        forName: "commands", contentWorld: .page)
    }
  }
}
