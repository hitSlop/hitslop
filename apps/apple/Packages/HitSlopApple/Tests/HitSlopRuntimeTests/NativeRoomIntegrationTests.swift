...
@MainActor func nativeCommandsRecoverThroughRealCloudflareRoom() async throws {
    ...
    let schema =
        #"{"type":"object","x-hitslop":{"version":1,"container":"map"},"properties":{"left":{"type":"integer"},"right":{"type":"integer"},"photo":{"type":"object","x-hitslop":{"container":"atomic","media":true},"properties":{"sha256":{"type":"string","pattern":"^[a-f0-9]{64}$"},"mime":{"type":"string"},"bytes":{"type":"integer","minimum":1}},"required":["sha256","mime","bytes"],"additionalProperties":false}},"required":["left","right"],"additionalProperties":true}"#
    ...
    let photoBytes = try #require(Data(base64Encoded: "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNk+A8AAQUBAScY42YAAAAASUVORK5CYII="))
    let aMedia = SlopMediaStore(directoryURL: aRoot.appendingPathComponent("stores/media"), rootURL: aRoot)
    let photo = try aMedia.add(photoBytes, imageOnly: true)
    func reference(_ media: SlopMediaStore.Reference) -> SlopDocumentJSON {
        .object(["sha256": .string(media.sha256), "mime": .string(media.mime), "bytes": .number(Double(media.bytes))])
    }
    #expect(try await a.apply(testRequest(a.openGuest(), ops: [testSet("photo", reference(photo))]))["ok"] == .bool(true))
    let seed = try await a.prepareSharing()
    ...
    let shared = try await owner.createDocument(
        id: seed["documentId"].string!, title: "Native room", slug: "native-room", schema: seed["schemaHash"].string!,
    ...
    let b = try #require(try SlopCommandDocument.open(package: SlopPackage(rootURL: bRoot), seed: seed))
    ...
    func change(_ doc: SlopCommandDocument, _ key: String, _ number: Double) async throws {
        let result = try await doc.apply(testRequest(doc.openGuest(), ops: [testSet(key, .number(number))]))
    ...
    let offline = try await b.apply(testRequest(b.openGuest(), ops: [testSet("right", .number(99))]))
    #expect(offline["error"]["code"].string == "offline")
    let aSession = SlopRoomSession(
        origin: origin, documentId: seed["documentId"].string!, schema: seed["schemaHash"].string!, document: a,
        credentials: { try await owner.session(documentId: seed["documentId"].string!) }, media: aTransfer.transport)
    let bSession = SlopRoomSession(
        origin: origin, documentId: seed["documentId"].string!, schema: seed["schemaHash"].string!, document: b,
        credentials: { try await guest.session(documentId: seed["documentId"].string!) }, media: bTransfer.transport)
    defer {
        aSession.stop()
        bSession.stop()
    }
    ...
    let bMedia = SlopMediaStore(directoryURL: bRoot.appendingPathComponent("stores/media"), rootURL: bRoot)
    ...
    let nextBytes = try #require(Data(base64Encoded: "R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7"))
    let nextPhoto = try bMedia.add(nextBytes, imageOnly: true)
    let changed = try await b.apply(testRequest(b.openGuest(), ops: [testSet("photo", reference(nextPhoto))]))
    ...
    return try JSONSerialization.data(withJSONObject: [
        ...
        "properties": [
            ...
            "$slop": [
                ...
                "properties": [
                    "format": ["const": 2], "documentId": identity, "schemaHash": identity, "authority": identity,
