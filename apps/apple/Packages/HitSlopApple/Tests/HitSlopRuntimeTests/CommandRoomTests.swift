...
@MainActor struct CommandRoomTests {
    ...
    @Test func depth64SurvivesReadyAndSnapshotMessages() async throws {
        ...
        func data(_ leaf: Int) -> SlopDocumentJSON {
            var value = SlopDocumentJSON.number(Double(leaf))
            for _ in 0..<63 { value = .object(["n": value]) }
            return .object(["value": value])
        }
        let source = try SlopDocumentJSON(data: Data(#"{"type":"object","x-hitslop":{"version":1,"container":"map"},"properties":{"value":{"x-hitslop":{"container":"atomic"}}}}"#.utf8))
        ...
        let session = SlopRoomSession(
            origin: URL(string: "http://127.0.0.1")!, documentId: identity.documentId, schema: identity.schema,
            document: document,
            credentials: { SlopRoomSessionToken(token: "token", documentId: identity.documentId, schema: identity.schema) },
            connect: { _ in socket })
        session.start()
        socket.deliver(["type": "welcome", "protocol": slopProtocolVersion, "peers": []])
        try await eventually { socket.frames("hello").count == 1 }
        socket.deliver(["type": "ready", "open": ["snapshot": try seed.jsonValue(), "lease": ["id": "deep-lease", "expiresAt": 9_999_999_999_999]], "peers": []])
        try await eventually { session.status == .live }
        var next = seed
        next["revision"] = .number(1)
        next["data"] = data(1)
        socket.deliver(["type": "snapshot", "snapshot": try next.jsonValue()])
        try await eventually { try await document.frame().data == data(1) }
        await session.stopAndWait()
        try await document.close()
        let reopened = try SlopCommandDocument(root: root, schema: source, initial: data(0))
        #expect(try await reopened.frame().data == data(1))
    ...
    @Test func lostReplyReconnectsAndResendsThePersistedRequest() async throws {
        ...
        let session = SlopRoomSession(
            ...
            credentials: {
                _ = calls.next()
                return SlopRoomSessionToken(token: "token", documentId: identity.documentId, schema: identity.schema)
            },
        ...
        let opening = SlopDocumentJSON.object([
            "snapshot": seed, "lease": .object(["id": .string("lease-one"), "expiresAt": .number(9_999_999_999_999)]),
        ])
        first.deliver(["type": "ready", "open": try opening.jsonValue(), "peers": []])
        try await eventually { session.status == .live }
        let request = testRequest(try await document.openGuest(), ops: [testSet("count", .number(1))])
        ...
        second.deliver([
            "type": "result", "authority": request["authority"].string!, "leaseId": request["leaseId"].string!,
