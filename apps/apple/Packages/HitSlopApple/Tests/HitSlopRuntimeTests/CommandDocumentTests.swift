...
struct CommandDocumentTests {
    private let schema = try! SlopDocumentJSON(
        data: Data(
    ...
    private func command(_ open: SlopDocumentJSON) -> SlopDocumentJSON {
        ...
        request["ops"] = .array([
            .object([
                "op": .string("increment"), "path": .array([.object(["key": .string("count")])]), "amount": .number(1),
            ])
        ])
    ...
    @Test func suspendedProposalDoesNotBlockCommandsOrClose() async throws {
        ...
        let document = try SlopCommandDocument(root: root, schema: schema, initial: .object(["count": .number(0)]))
        let open = try await document.openGuest()
        try await document.flush()
        let file = root.appendingPathComponent("stores/data.json")
        var stale = try SlopDocumentJSON(data: Data(contentsOf: file))
        ...
        let reopened = try SlopCommandDocument(root: root, schema: schema, initial: .object(["count": .number(0)]))
        #expect(try await reopened.frame().data["count"] == .number(2))
        try await reopened.discardExternalProposal()
        #expect(try SlopDocumentJSON(data: Data(contentsOf: file))["data"]["count"] == .number(2))
    ...
    @Test func realProjectionFailureBlocksCloseWithoutRejectingCommittedCommand() async throws {
        ...
        try FileManager.default.removeItem(at: file)
        try await document.close()
        #expect(try SlopDocumentJSON(data: Data(contentsOf: file))["data"]["count"] == .number(1))
    }
    @Test func sharedModeSurvivesRestartAndCannotBecomeLocallyWritable() async throws {
        let root = try directory()
        defer { try? FileManager.default.removeItem(at: root) }
        let document = try SlopCommandDocument(root: root, schema: schema, initial: .object(["count": .number(0)]))
        _ = try await document.openGuest()
        let seed = try await document.prepareSharing()
        #expect(try await document.frame().writable == false)
        let opening = SlopDocumentJSON.object([
            "snapshot": seed, "lease": .object(["id": .string("room-lease"), "expiresAt": .number(9_999_999_999_999)]),
        ])
        try await document.connect(opening: opening) { _ in .object(["ok": .bool(true), "revision": .number(1)]) }
        try await document.disconnect()
        try await document.close()
        let reopened = try SlopCommandDocument(root: root, schema: schema, initial: .object(["count": .number(0)]))
        let open = try await reopened.openGuest()
    ...
    @Test func pendingSharedAttemptSurvivesAnOwnerRestart() async throws {
        ...
        let original = command(try await document.openGuest())
    ...
    @Test func promotionResumesWithTheSameFrozenSeedAfterRestart() async throws {
        ...
        #expect(try await reopened.prepareSharing() == seed)
        #expect(try await reopened.frame().writable == false)
        try await reopened.close()
    }
    @Test func anotherWindowCannotProjectAnOlderCommittedDocument() async throws {
        let root = try directory()
        defer { try? FileManager.default.removeItem(at: root) }
        let a = try SlopCommandDocument(root: root, schema: schema, initial: .object(["count": .number(0)]))
        let b = try SlopCommandDocument(root: root, schema: schema, initial: .object(["count": .number(0)]))
        let first = try await a.openGuest()
        let second = try await b.openGuest()
        ...
        #expect(
            try SlopDocumentJSON(data: Data(contentsOf: root.appendingPathComponent("stores/data.json")))["data"][
                "count"] == .number(2))
        try await a.close()
        try await b.close()
    }
    ...
    @Test func committedExternalImportReplaysAfterInterruptedBookkeeping() async throws {
        ...
        await document.shutDown()
        let file = root.appendingPathComponent("stores/data.json")
        var envelope = try SlopDocumentJSON(data: Data(contentsOf: file))
        envelope["data"]["count"] = .number(42)
        let bytes = try envelope.encoded(), hash = SlopCommandDocument.digest(bytes)
        try bytes.write(to: file, options: .atomic)
        let storage = try SlopCommandStorage(root: root)
        let opening = try storage.open()
        var record = try #require(try storage.load())
        record.metadata.externalOpening = .init(authority: opening["snapshot"]["authority"].string!, lease: opening["lease"])
        try storage.transaction { try storage.saveMetadata(record.metadata) }
        var request = command(opening)
        var fields = request.object
        fields.removeValue(forKey: "ops")
        request = .object(fields)
        request["requestId"] = .string("file:" + hash)
        request["replace"] = .object(["baseRevision": envelope["$slop"]["baseRevision"], "data": envelope["data"]])
        #expect(try storage.apply(request, schema: SlopDocumentSchema(schema)).result["ok"] == .bool(true))
        // Simulate termination here: the commit is durable, but materializedHash is still old.
        let reopened = try SlopCommandDocument(root: root, schema: schema, initial: .object(["count": .number(0)]))
        _ = try await reopened.openGuest()
...
private actor AttemptLog {
    var values: [SlopDocumentJSON] = []
    func append(_ request: SlopDocumentJSON) { values.append(request) }
}
