...
private actor RecordingTransport: ClientTransport {
    struct Sent: Sendable { let request: HTTPRequest; let body: Data }
    var sent: [Sent] = []
    func send(_ request: HTTPRequest, body: HTTPBody?, baseURL: URL, operationID: String) async throws -> (HTTPResponse, HTTPBody?) {
        let bytes: Data
        if let body { bytes = try await Data(collecting: body, upTo: 1024 * 1024) } else { bytes = Data() }
        ...
        return (HTTPResponse(status: status, headerFields: [.contentType: "application/json"]), HTTPBody(payload))
    }
}
...
@Suite struct CloudAPITests {
    @Test func generatedMultipartAndBinaryRequestsPreserveCredentialsAndBytes() async throws {
        ...
        #expect(requests[0].request.headerFields[.contentType]?.hasPrefix("multipart/form-data; boundary=") == true)
...
private actor CatalogTransport: ClientTransport {
    ...
    func send(_ request: HTTPRequest, body: HTTPBody?, baseURL: URL, operationID: String) async throws -> (HTTPResponse, HTTPBody?) {
        ...
        if operationID == "recordCreation" {
            ...
            return (.init(status: .ok, headerFields: [.contentType: "application/json"]), HTTPBody(#"{"ok":true}"#))
        }
        let secondPage = request.path?.contains("cursor=") == true
        let hash = String(repeating: "a", count: 64)
        let asset: [String: Any] = ["url": "https://example.com/api/artifact?key=artifacts/sha256/\(hash).slop.zip", "sha256": hash, "bytes": 1]
        let entry: [String: Any] = [
            ...
            "release": ["number": 1, "publishedAt": secondPage ? "2026-01-01T00:00:00Z" : "2026-01-01T00:00:00.000Z"],
            "preview": asset, "icon": asset, "download": asset,
        ]
        let page: [String: Any] = ["version": 1, "templates": [entry], "nextCursor": secondPage && !repeatingCursor ? NSNull() : "next"]
        return (.init(status: .ok, headerFields: [.contentType: "application/json"]), HTTPBody(try JSONSerialization.data(withJSONObject: page)))
    }
}
...
@Suite struct CatalogAPITests {
    ...
    @Test func rejectsPaginationLoopsInsteadOfReturningPartialCatalog() async throws {
        let api = SlopCloudAPI(origin: URL(string: "https://example.com")!, transport: CatalogTransport(repeatingCursor: true))
