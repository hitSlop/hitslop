...
private func crashDocument(_ root: URL) throws -> SlopCommandDocument {
  try SlopCommandDocument(
    root: root, schema: FixtureJSON(data: Data(crashSchema.utf8)),
    initial: .object(["count": .number(0)]))
}

@Test(arguments: [1, 2, 3]) func databaseRequiresCurrentFormatAndPreservesRejectedBytes(_ version: Int)
  throws
{
...
func localCrashWriter() async throws {
  let root = URL(
    fileURLWithPath: try #require(ProcessInfo.processInfo.environment["HITSLOP_CRASH_ROOT"]))
  let document = try crashDocument(root)
  _ = try await document.apply(
    testRequest(document.fixtureOpening(), ops: [testSet("count", .number(42))]))
