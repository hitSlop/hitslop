...
struct DocumentOpsTests {
  private func corpus() throws -> FixtureJSON {
    try FixtureJSON(
      data: Data(
  ...
  @Test func canonicalCorpus() throws {
    ...
    let schema = try FixtureSchema(fixture["schema"])
    ...
    for scenario in fixture["scenarios"].array {
      ...
      for event in scenario["events"].array {
        ...
        #expect(
          outcome.snapshot["revision"] == event["expected"]["revision"], "\(scenario["name"])")
        #expect(outcome.snapshot["data"] == event["expected"]["data"], "\(scenario["name"])")
        #expect(
          outcome.result["error"]["code"] == event["expected"]["error"], "\(scenario["name"])")
      }
    }
  }
  @Test func durableReceiptAndRollback() throws {
    ...
    try storage.initialize(snapshot: snapshot)
    let open = try storage.open(now: 0)
    var request = fixture["scenarios"].array.first { $0["name"].string == "increment" }!["events"]
      .array[0][
        "request"]
    request["leaseId"] = open["lease"]["id"]
    ...
    #expect(
      try reopened.apply(request, schema: schema, now: expired).result["error"]["code"].string
        == "lease_expired")
  }
  @Test func twoConnectionsSerializeLatestState() throws {
    ...
    #expect(
      try second.apply(request, schema: schema, now: 2).snapshot["data"]["count"] == .number(4))
  }
  @Test func receiptRowsContainOnlyFixedSizeDigests() throws {
    ...
    #expect(
      sqlite3_prepare_v2(
        db, "SELECT count(*), min(length(digest)), max(length(digest)) FROM receipts", -1,
  ...
  @Test func undoIsBoundedDurableAndAtomicAcrossConnections() throws {
    ...
    func undoCount() throws -> Int32 {
      ...
      else { throw SlopDocumentError("Cannot count undo slots") }
    ...
    let command = FixtureJSON.object(undo)
    var foreign = command
    foreign["leaseId"] = try second.open(now: 1)["lease"]["id"]
    #expect(
      try second.apply(foreign, schema: schema, now: 1).result["error"]["code"]
        == .string("rejected"))
    ...
    #expect(
      try second.apply(.object(undo), schema: schema, now: 6).result["error"]["code"]
        == .string("stale_revision"))
    _ = try first.open(now: slopRetryWindowMilliseconds)
    #expect(try undoCount() == 0)
    #expect(
      try second.apply(command, schema: schema, now: slopRetryWindowMilliseconds).result["error"][
        "code"] == .string("lease_expired"))
  }
  ...
  @Test func missingOrUnknownAuthorityModeIsRejected() throws {
    ...
    try storage.initialize(snapshot: snapshot)
    for metadata in [
      #"{"pendingProjection":false,"outstanding":{}}"#,
      #"{"mode":"unknown","pendingProjection":false,"outstanding":{}}"#,
    ] {
  ...
  @Test func unknownAnnotationsAreRejected() throws {
    let markers: [FixtureJSON] = [
      .string("invalid"), .object([:]), .object(["container": .string("unknown")]),
    ]
    for marker in markers {
      let schema = FixtureJSON.object([
        "type": .string("object"),
        "x-hitslop": .object(["version": .number(1), "container": .string("map")]),
        "properties": .object(["value": .object(["type": .string("object"), "x-hitslop": marker])]),
      ])
      #expect(throws: (any Error).self) { try FixtureSchema(schema) }
    }
  }
