...
private let crashSchema = #"{"type":"object","x-hitslop":{"version":1,"container":"map"},"properties":{"count":{"type":"integer"}},"required":["count"],"additionalProperties":true}"#
private func crashDocument(_ root: URL) throws -> SlopLoroDocument {
    try SlopLoroDocument(root: root, schema: SlopDocumentJSON(data: Data(crashSchema.utf8)), initial: .object(["count": .number(0)]))
}

@Test func databaseRequiresV1AndPreservesRejectedBytes() throws {
    ...
    do { let storage = try SlopSyncStorage(root: root); try storage.execute("PRAGMA user_version=2") }
...
func localCrashWriter() async throws {
    ...
    _ = try await document.apply(.init(session: "crash", sequence: 1, base: frame.revision, after: .object(["count": .number(42)])))
    // Publication has acknowledged the committed edit; no close/flush follows.
    try Data(String(ProcessInfo.processInfo.processIdentifier).utf8).write(to: root.appendingPathComponent("committed.pid"), options: .atomic)
    try await Task.sleep(for: .seconds(30))
    Issue.record("The parent must terminate this writer after its acknowledged commit")
}

@Test(.enabled(if: ProcessInfo.processInfo.environment["HITSLOP_LOCAL_TESTS"] == "1" && ProcessInfo.processInfo.environment["HITSLOP_CRASH_CHILD"] == nil))
func acknowledgedEditSurvivesProcessTermination() async throws {
    ...
    for argument in CommandLine.arguments.dropFirst() {
        ...
        if argument == "--filter" || argument == "--skip" { skipValue = true; continue }
        arguments.append(argument)
    }
    process.arguments = arguments + ["--filter", "localCrashWriter"]
    var environment = ProcessInfo.processInfo.environment
    environment["HITSLOP_CRASH_CHILD"] = "1"; environment["HITSLOP_CRASH_ROOT"] = root.path
    ...
    while !FileManager.default.fileExists(atPath: marker.path) {
        guard process.isRunning, ContinuousClock.now < deadline else { throw SlopDocumentError("Crash writer never acknowledged its edit") }
