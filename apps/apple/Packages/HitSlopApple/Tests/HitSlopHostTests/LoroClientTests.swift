...
@Suite(.serialized) struct LoroClientTests {
  ...
  @Test @MainActor func realWindowLiveCLIExportAndClosedCLI() async throws {
    ...
    for name in ["Checklist", "Expenses"] {
      ...
      panel.orderOut(nil)
      _ = try await controller.session.webView.callAsyncJavaScript(
        "const input=document.querySelector('textarea[aria-label=\"Checklist title\"],input');for (const value of ['abcX','abcXY','abcXYZ']) {input.value=value;input.dispatchEvent(new Event('input',{bubbles:true}))} await globalThis.__slop.flush();return true",
        arguments: [:], in: nil, contentWorld: .page)
  ...
  @Test @MainActor func executableExportsLiveSelectionAndClosedDefaultView() async throws {
    ...
    #expect(
      try await view.evaluateJavaScript(
        "document.querySelector('[role=tab][aria-selected=true]').textContent.includes('Filed')")
        as? Bool == true)
    #expect(
      try await view.evaluateJavaScript(
    ...
    #expect(result.0 == 0, "\(result.2)")
    #expect(
      PDFDocument(data: try Data(contentsOf: closed))?.string?.contains("Filed tasks") == false)
  ...
  @Test @MainActor func inlineCaptureTargetsAreLazyAndUseCurrentDocument() async throws {
    ...
    _ = try await view.callAsyncJavaScript(
      "[...document.querySelectorAll('button')].find(b=>b.textContent.includes('File finished')).click(); await globalThis.__slop.flush(); return true",
      arguments: [:], in: nil, contentWorld: .page)
    let filed = try await DocumentCommand.run(method: "get", url: root)
    #expect(
      String(decoding: filed, as: UTF8.self).components(separatedBy: "\"archived\" : true").count
        == 4)
    // Use the accessible tab name rather than a Bits UI implementation attribute.
    _ = try await view.callAsyncJavaScript(
      "[...document.querySelectorAll('[role=tab]')].find(b=>b.textContent.includes('Filed')).click(); return true",
      arguments: [:], in: nil, contentWorld: .page)
