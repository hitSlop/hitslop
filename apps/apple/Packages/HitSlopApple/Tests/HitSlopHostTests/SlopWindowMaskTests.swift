...
@Test @MainActor func transparentGeometryInstallsAClearBacking() throws {
    ...
    let manifest = #"{"$schema":"https://api.hitslop.com/schemas/v1/manifest.schema.json","runtime":"1.0.0","author":{"name":"Fixture Author","url":"https://example.com"},"slug":"transparent","title":"Transparent","description":"Tests transparent geometry.","categories":["utilities"],"presentation":{"width":240,"height":180,"background":"transparent"}}"#
...
private func maskedFixture() throws -> URL {
    ...
    try Data("<html></html>".utf8).write(to: root.appendingPathComponent("app.html"))
    let manifest = #"{"$schema":"https://api.hitslop.com/schemas/v1/manifest.schema.json","runtime":"1.0.0","author":{"name":"Fixture Author","url":"https://example.com"},"slug":"asymmetric","title":"Asymmetric","description":"Tests image mask orientation.","categories":["utilities"],"presentation":{"width":240,"height":180,"skin":"assets/window-mask.png"}}"#
    try Data(manifest.utf8).write(to: root.appendingPathComponent("manifest.json"))
    try writeCanonicalDocumentSkill(to: root)
