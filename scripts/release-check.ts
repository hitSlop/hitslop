...
function assertNoGeneratedSource(files: string[]): void {
  const generated = files.filter(
    (path) => /^packages\/[^/]+\/src\//.test(path) && /\.(?:d\.ts|js)$/.test(path),
  );
  if (generated.length)
    throw new Error(
...
async function assertDocumentationLinks(files: string[]): Promise<void> {
  const markdown = files.filter(
    (path) =>
      path.endsWith(".md") && !path.startsWith("archive/") && !path.startsWith("examples/archive/"),
  );
  const failures: string[] = [];
...
async function main(): Promise<void> {
  ...
  await Promise.all([
    ...
    checkIgnored("apps/cloudflare/.env", true),
  ...
  if (process.argv.includes("--hygiene-only")) return;
  ...
  if (process.platform === "darwin" && process.env.HITSLOP_SKIP_SWIFT !== "1") {
    ...
    await command(["bun", "scripts/native-runtime-check.ts"], root, false, swiftEnv);
    await command(
      [
        "swift",
        "build",
