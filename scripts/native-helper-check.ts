...
try {
  ...
  await cp(join(root, "examples/slops/quick-checklist/dist/quick-checklist.slop"), document, {
    recursive: true,
  });
  const output = join(temporary, "preview.png");
  await run([
    join(app, "Contents/Helpers/hitslop-native"),
    "screenshot",
    document,
    "--output",
    output,
  ...
  if (bytes.length < 8 || bytes.subarray(0, 8).toString("hex") !== "89504e470d0a1a0a")
    throw new Error("Embedded helper did not render a PNG");
  process.stdout.write("✓ embedded helper renders without SwiftPM build resources\n");
} finally {
  if (process.env.HITSLOP_KEEP_RELEASE_TEMP === "1")
    process.stdout.write(`Helper diagnostics retained: ${temporary}\n`);
