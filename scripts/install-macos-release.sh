...
for required in "$app" "$native_resources" "$app/Contents/Helpers/HitSlopApple_HitSlopRuntime.bundle" "$app/Contents/Helpers/HitSlopApple_HitSlopDocumentEngine.bundle" "$provisioning_profile"; do
  if [ ! -e "$required" ]; then
