...
for required in "$app" "$app/Contents/Helpers/HitSlopApple_HitSlopRuntime.bundle" "$app/Contents/Helpers/HitSlopApple_HitSlopWasm.bundle" "$provisioning_profile"; do
  if [ ! -e "$required" ]; then
