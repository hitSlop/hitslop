...
async function main(): Promise<void> {
    ...
    for (const group of [
      ...
      for (const [dependency, reference] of Object.entries(
        ...
        if (typeof reference === "string" && reference.startsWith("workspace:")) {
          throw new Error(
            `${packagePath}: ${group}.${dependency} must use a publishable semver range, not ${reference}`,
          );
        }
      }
    }
  ...
  for (const name of packageNames)
    await command(["bun", "run", "build"], join(root, "packages", name));
  for (const name of packageNames)
    await command(["bun", "run", "check"], join(root, "packages", name));
  for (const name of packageNames)
    await command(["bun", "run", "test"], join(root, "packages", name));

  // Cloudflare is the untrusted package-ingress boundary, so its validator
  // is part of the npm foundation even though the app itself is not published.
  await command(["bun", "run", "check"], join(root, "apps/cloudflare"));
  const syncEnv: Record<string, string> = {};
  if (process.env.HITSLOP_NATIVE_SYNC === "1") {
    await command(["bun", "slop", "build", "examples/slops/quick-checklist"]);
    syncEnv.HITSLOP_PILOT_PACKAGE = join(
      root,
      "examples/slops/quick-checklist/dist/quick-checklist.slop",
    );
  }
  await command(["bun", "run", "test"], join(root, "apps/cloudflare"), false, syncEnv);
  ...
  const requestedRoot = resolve(process.env.RUNNER_TEMP ?? tmpdir());
  const relativeRoot = relative(root, requestedRoot);
  const temporaryRoot = relativeRoot.startsWith("..") ? requestedRoot : tmpdir();
  ...
  try {
    ...
    const packageReferences: Record<(typeof packageNames)[number], string> = {} as Record<
      (typeof packageNames)[number],
      string
    >;
    for (const name of packageNames) {
      ...
      const allowed = new Set([
        ...
        "LICENSE",
        "dist",
        ...(name === "cli" ? ["templates", "skills"] : []),
        ...(["schema", "api"].includes(name) ? ["generated"] : []),
      ]);
      for (const entry of entries) {
        const path = entry.replace(/^package\//, "").replace(/\/$/, "");
        if (!path) continue;
        if (!allowed.has(path.split("/")[0]!))
          throw new Error(`Unexpected packed file: ${name}/${path}`);
      }
      if (name === "cli" && !entries.includes("package/dist/module-loader.js"))
        throw new Error("CLI tarball is missing its module loader");
    ...
    await command(
      [
        join(harness, "node_modules/.bin/slop"),
        "init",
        starter,
        "--yes",
        "--author-name",
    ...
    starterPackage.overrides = Object.fromEntries(
      packageNames.map((name) => [`@hitslop/${name}`, packageReferences[name]]),
    );
    ...
    await cp(
      join(root, "packages/cli/tests/fixtures/counter.typecheck.txt"),
      join(starter, "src/counter.typecheck.ts"),
    );
    ...
    await command(
      [
        join(starter, "node_modules/.bin/slop"),
        "validate",
        join(starter, "dist/release-counter.slop"),
      ],
      starter,
    );
    ...
    if (process.env.HITSLOP_NATIVE_CLI) {
      ...
      await command(
        ...
        root,
        false,
        { HITSLOP_STARTER_PACKAGE: builtStarter },
      );
      for (const target of ["preview", "icon"]) {
        const output = join(temporary, `counter-${target}.png`);
        await command(
          [slop, "screenshot", builtStarter, "--target", target, "--output", output],
          starter,
        );
        const png = await readFile(output);
        if (png.readUInt32BE(0) !== 0x89504e47) throw new Error("Invalid native PNG");
        if (target === "icon" && (png.readUInt32BE(16) !== 512 || png.readUInt32BE(20) !== 512))
          throw new Error("Icon must be 512×512");
      }
      for (const format of ["png", "pdf"]) {
        const output = join(temporary, `counter-export.${format}`);
        await command(
          [slop, "export", builtStarter, "--format", format, "--output", output],
          starter,
        );
        ...
        if (
          ...
          throw new Error(`Invalid ${format} export`);
      }
      if ((await readdir(builtStarter)).includes("stores"))
        throw new Error("Capture mutated the starter template");
    }
    ...
    for (const name of [
      ...
      await cp(join(root, "examples/slops/quick-checklist", name), join(app, name), {
        recursive: true,
      });
    }
    const examplePackage = JSON.parse(
      await readFile(join(root, "examples/slops/package.json"), "utf8"),
    ) as {
      dependencies: Record<string, string>;
      devDependencies: Record<string, string>;
    };
    const appPackage = {
      name: "hitslop-capture-consumer",
      private: true,
      type: "module",
      scripts: {
      ...
      JSON.stringify(
        {
          ...
          include: ["src/**/*", "schema.ts", "initial.ts", "theme.ts"],
        },
        null,
        2,
      ) + "\n",
    ...
    await command(
      [
        join(app, "node_modules", ".bin", "slop"),
        "validate",
        join(app, "dist", `${manifest.slug}.slop`),
      ],
      app,
    );
  } finally {
    if (process.env.HITSLOP_KEEP_RELEASE_TEMP === "1")
      process.stdout.write(`Release diagnostics retained: ${temporary}\n`);
