<script lang="ts">
  ...
  import { documentContext, Slop, bindText, useDocument } from "@hitslop/document/svelte";
  ...
  type StopKey = "slow" | "takeNote" | "openUp" | "pursue";
  type StringList = {
    preview: (index: number, value: string) => void;
    set: (index: number, value: string) => void;
  };

  const host = getContext<{ subscribe: (listener: () => void) => () => void }>(documentContext);
  if (!host) throw new Error("Host document missing");
  ...
  const STOP_STEPS: { key: StopKey; letter: string; title: string; hint: string; icon: typeof Turtle }[] = [
    { key: "slow", letter: "S", title: "S — Slow down", hint: "Pause. Take a deep breath. Let your body settle.", icon: Turtle },
    { key: "takeNote", letter: "T", title: "T — Take note", hint: "What am I thinking, feeling, or urging right now?", icon: Search },
    { key: "openUp", letter: "O", title: "O — Open up", hint: "Make room for what’s here, without fighting or numbing.", icon: Heart },
    { key: "pursue", letter: "P", title: "P — Pursue values", hint: "What kind of person do I want to be right now?", icon: Star },
  ];
  ...
  const COMMON_VALUES = ["Humility", "Courage", "Craft", "Kindness", "Honesty", "Curiosity", "Patience"];
  ...
  function bindStringList(node: HTMLInputElement, params: { index: number; list: StringList; read: () => string }) {
    let { index, list, read } = params;
    let editing = false;
    const sync = () => {
      if (editing) return;
      const value = read();
    ...
    const stop = host.subscribe(sync);
    sync();
    return {
      update(next: { index: number; list: StringList; read: () => string }) {
        index = next.index;
        list = next.list;
        read = next.read;
        sync();
  ...
  const showIntro = $derived(!capture.isRenderer() && (!doc.current.introDismissed || introOpen));
  const currentTheme = $derived(doc.current.theme);
  const hooks = $derived(doc.current.hooks);
  const values = $derived(doc.current.values);
  const awayMoves = $derived(doc.current.awayMoves);
  const towardsMoves = $derived(doc.current.towardsMoves);
  const stop = $derived(doc.current.stop);
  ...
  function onDateSelect(val: DateValue | undefined) {
    ...
    doc.fields.date.set(d.toLocaleDateString(undefined, {
      weekday: "short",
      month: "short",
      day: "numeric",
      year: "numeric",
  ...
  function dismissIntro() {
    doc.fields.introDismissed.set(true);
    introOpen = false;
    introStep = 0;
  }
    ...
    <div class="masthead-meta">
      ...
      <Popover.Root bind:open={calendarOpen}>
        <Popover.Trigger class="date-trigger" aria-label="Select date">
          ...
          <span>Date:</span>
          <strong>{doc.current.date}</strong>
        </Popover.Trigger>
        ...
        <div class="guided-body">
          <section class="card card-situation">
            ...
            <textarea
              ...
              rows={3}
              use:bindText={doc.fields.situation}
              use:autosize
            ...
            <div class="chips-wrap">
              {#each hooks as hook, index (index)}
                <div class="chip-item">
                  <input
                    class="chip-input"
                    use:bindStringList={{ index, list: doc.fields.hooks, read: () => doc.current.hooks[index] ?? "" }}
            ...
            <div class="chips-wrap">
              {#each values as value, index (index)}
                <div class="chip-item">
                  <input
                    class="chip-input"
                    use:bindStringList={{ index, list: doc.fields.values, read: () => doc.current.values[index] ?? "" }}
            ...
            <div class="commit-col">
              ...
              <div class="commit-content">
                ...
                <textarea
                  class="commit-input"
                  rows={2}
                  use:bindText={doc.fields.nextMove}
                  use:autosize
            ...
            <div class="commit-right">
              <div class="commit-col">
                ...
                <div class="commit-content">
                  ...
                  <input
                    class="commit-input"
                    use:bindText={doc.fields.nextWhen}
              ...
              <label class="commit-done-label">
                <Checkbox.Root
                  checked={doc.current.nextDone} onCheckedChange={(checked) => doc.fields.nextDone.set(checked === true)}
                  class="done-box"
...
{#snippet exportView()}
  <main class="sheet" class:theme-dark={currentTheme === "dark"}>
    <header class="masthead">
      <div class="masthead-center">
        <h1>Choice Point</h1>
        <p>{doc.current.date}</p>
      </div>
    </header>
    <div class="full-map-container" style="display: flex; flex-direction: column; gap: 16px;">
      <section class="card card-situation">
        <h2 class="card-title">Challenging situation</h2>
        <p class="situation-textarea">{doc.current.situation}</p>
      </section>
      <section class="card card-hooks">
        <h2 class="card-title">Hooks</h2>
        <div class="chips-wrap">
          {#each doc.current.hooks as hook, index (index)}<div class="chip-item"><span class="chip-input">{hook}</span></div>{/each}
        </div>
      </section>
      <section class="card card-away">
        <h2 class="card-title">Away moves</h2>
        <ul class="moves-list">
          {#each doc.current.awayMoves as move (move.$id)}<li class="move-item"><span class="move-bullet">•</span><p class="move-textarea">{move.text}</p></li>{/each}
        </ul>
      </section>
      <section class="card card-towards">
        <h2 class="card-title">Toward moves</h2>
        <ul class="moves-list">
          {#each doc.current.towardsMoves as move (move.$id)}<li class="move-item"><span class="move-bullet">•</span><p class="move-textarea">{move.text}</p></li>{/each}
        </ul>
      </section>
      <section class="card card-values">
        <h2 class="card-title">Values</h2>
        <div class="chips-wrap">
          {#each doc.current.values as value, index (index)}<div class="chip-item"><span class="chip-input">{value}</span></div>{/each}
        </div>
      </section>
      <section class="commit-card">
        <p class="commit-title">{doc.current.nextMove}</p>
        <p class="commit-hint">{doc.current.nextWhen}{doc.current.nextDone ? " · Done" : ""}</p>
      </section>
    </div>
  </main>
{/snippet}
...
{#snippet icon()}
<section class="cp-render cp-icon" data-slop-render="icon" aria-hidden="true">
  <article class="icon-page">
    <svg viewBox="0 0 240 240" role="presentation">
      <defs>
        <marker id="icon-away-arr" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
          <path d="M0 0 L6 3 L0 6 z" fill="#d95d43" />
        </marker>
        <marker id="icon-towards-arr" markerWidth="6" markerHeight="6" refX="5" refY="3" orient="auto">
          <path d="M0 0 L6 3 L0 6 z" fill="#2d9472" />
        </marker>
      </defs>
      <!-- Base road stem -->
      <path class="icon-stem-road" d="M120 220 L120 140" />
      <path class="icon-stem-dash" d="M120 214 L120 148" />

      <!-- Left / Away path -->
      <path class="icon-away" marker-end="url(#icon-away-arr)" d="M120 140 C92 110 68 82 46 48" />

      <!-- Right / Towards path -->
      <path class="icon-towards" marker-end="url(#icon-towards-arr)" d="M120 140 C148 110 172 82 194 48" />

      <!-- Central signpost medallion -->
      <circle class="icon-node-outer" cx="120" cy="140" r="28" />
      <circle class="icon-node-inner" cx="120" cy="140" r="22" />

      <!-- Mini signpost glyph in medallion -->
      <path class="icon-sign-post" d="M120 126 L120 154" />
      <path class="icon-sign-bar1" d="M110 132 L130 132" />
      <path class="icon-sign-bar2" d="M112 144 L128 144" />
    </svg>
  </article>
</section>
{/snippet}
