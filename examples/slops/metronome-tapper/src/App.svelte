<script lang="ts">
  ...
  const weight = $derived(`${((store.current.bpm - 40) / 200) * 58 + 18}%`);
  ...
  $effect(() => {
    ...
    const frozen = freezeSwing;
    if (frozen || reduced || !playing) {
      if (!frozen) void pendulum.set(0, { instant: true });
      return;
    }
    let cancelled = false;
    ...
    const beat = () => {
      if (cancelled) return;
      dir *= -1;
      const bpm = Math.max(40, store.current.bpm);
  ...
  onMount(() => {
    ...
    return capture.onPrepare(async (_mode, signal) => {
      freezeSwing = true;
      await pendulum.set(CAPTURE_ANGLE, { instant: true });
      await tick();
...
<main class="metronome-shell" data-slop-selection="none" aria-busy={store.isLoading} aria-label="Metronome">
...
<IconTarget><Icon bpm={store.current.bpm} tempo={tempoName} /></IconTarget>
<ExportTarget>
  <Export
    bpm={store.current.bpm}
    tempo={tempoName}
    signature={store.current.signature}
    muted={store.current.muted}
    {beatsPerBar}
    {weight}
  />
</ExportTarget>
