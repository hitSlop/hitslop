...
if (false) {
  const store = createDocument({ schema, initial: { title: "Typed", tasks: [] } });
  ...
  createDocument({ schema, initial: { title: 1, tasks: [] } });
  // @ts-expect-error nested edits remain schema-typed
  store.data.tasks.push({ id: "bad", text: "bad", done: "yes", archived: false });
  // @ts-expect-error required fields remain required
  createDocument({ schema, initial: { tasks: [] } });
}
