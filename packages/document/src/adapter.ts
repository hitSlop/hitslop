...
/** Framework adapters receive the existing host-owned document, never open another. */
export interface ViewAdapter<N extends ObjectNode = ObjectNode> {
  mount(context: {
    document: Document<N>;
    target: HTMLElement;
  }): DocumentView | Promise<DocumentView>;
}

/** Load the shared runtime and connect a framework view to the native lifecycle. */
export async function mountDocumentView(adapter: ViewAdapter): Promise<void> {
  ...
  try {
    ...
    if (!native) {
      ...
      if (manifest?.presentation)
        config.presentation = runtime.presentationStage(manifest.presentation);
    }
    if (native)
    ...
    (globalThis as any).__slop = await mountViewLifecycle({
      ...
      session,
      capture: runtime.captureController(),
      recovered: native ? () => runtime.hostCall({ method: "runtimeRecovered" }) : undefined,
    });
