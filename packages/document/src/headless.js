import { initialize, Document, Session, HostStore, hostCall, fromDescriptor } from './index.js';
try {
  await initialize();
  ...
  globalThis.__slop = {
    request: request => session.handle({...request,schemaHash:request.schemaHash ?? doc.key}),
    flush: () => session.flush(),
    close: () => session.close(),
    retrySave: () => session.flush(),
  };
  await hostCall({method:'ready'});
} catch (error) { await hostCall({method:'failed',error:String(error)}); }
