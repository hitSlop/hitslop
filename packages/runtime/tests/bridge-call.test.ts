...
test("bridge calls validate parameters before sending and infer method results", async () => {
  ...
  const result = await call("media.write", { name: "hero", data: "YQ==", mimeType: "image/png" });
  expect(result.revision).toBe("r1");
  expect(sent).toEqual([{ method: "media.write", name: "hero", data: "YQ==", mimeType: "image/png" }]);
  ...
  if (false) {
    ...
    await call("sync.open", { value: 1 });
    // @ts-expect-error result is a revision object, not a count
    const count: number = await call("media.write", { name: "hero", data: "YQ==", mimeType: "image/png" });
    void count;
  }
});
...
test("bridge rejects malformed envelopes and mismatched successful results", async () => {
  for (const reply of [
    ...
    await expect(call("media.write", { name: "hero", data: "YQ==", mimeType: "image/png" })).rejects.toThrow("validation failed");
  }
});
...
test("bridge preserves structured native errors and valid nested JSON", async () => {
  ...
  await expect(failed("media.write", { name: "hero", data: "YQ==", mimeType: "image/png" })).rejects.toMatchObject({ code: "revision_conflict", message: "Changed on disk" });
  const call = createBridgeCall(async () => ({ ok: true, value: { exists: true, revision: "r2" } }));
  expect(await call("media.open", {name: "hero"})).toEqual({ exists: true, revision: "r2" });
});
