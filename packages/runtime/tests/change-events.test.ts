...
test("invalid host events cannot break dispatch; listener failures are isolated", () => {
  ...
  listeners.sync.add(() => { throw new Error("listener failed"); });
  listeners.sync.add(event => received.push(event));
  for (const value of [null, {}, { kind: "invented" }, { kind: "sync", sequence: "1" }]) dispatchChange(value, listeners, error => errors.push(error));
  expect(received).toHaveLength(0);
  expect(errors).toHaveLength(4);
  const event: SlopChange = { kind: "sync", sequence: 1, source: "external", revision: null };
