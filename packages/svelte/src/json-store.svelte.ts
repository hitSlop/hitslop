...
export class JsonStore<T> {
  ...
  constructor(private storeID: string, fallback: T) {
    this.current = clone(fallback);
    this.queue = new JsonWriteQueue({ fallback, clone, read: () => slop.jsonRead<T>(storeID), write: (value, revision) => slop.jsonWrite(storeID, value, revision), onValue: (value) => { this.current = value; }, onRevision: (revision) => { this.revision = revision; }, onSource: (source) => { this.lastChangeSource = source; }, onError: (message) => { this.error = message; } });
    void this.reload(); this.unwatch = slop.watch("json", storeID, (event) => { if (event.revision !== this.queue.revision) void this.reload(); });
  }
...
export const jsonStore = <T>(storeID: string, fallback: T): JsonStore<T> => new JsonStore(storeID, fallback);
