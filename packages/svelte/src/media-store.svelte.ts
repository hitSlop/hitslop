...
export class MediaStore<Fallback extends string | null> {
  ...
  constructor(
    ...
    this.unwatch = slop.media.onChange((event) => {
      if (!event.name || event.name === name) void this.reload();
    });
    void this.reload();
  }
  choose(): void {
    chooseLocalFile(this.accept, (file) => {
      void this.replace(file).catch(() => undefined);
    });
  }
  async replace(file: File): Promise<void> {
    ...
    await this.mutate(async () => {
      ...
      const result = await slop.media.write(
        this.name,
        data,
  ...
  private async mutate(
    operation: () => Promise<{ exists: boolean; revision: string | null }>,
  ): Promise<void> {
