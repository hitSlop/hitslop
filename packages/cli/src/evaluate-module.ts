  ...
  try {
    const message = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(
          () => reject(new Error(`${kind} evaluation timed out after ${timeoutMs}ms`)),
          timeoutMs,
        );
      }),
    ]);
    ...
    throw new Error(
      `Could not evaluate ${kind}.ts (${absolute}): ${error instanceof Error ? error.message : String(error)}`,
      { cause: error },
    );
  } finally {
    clearTimeout(timer);
