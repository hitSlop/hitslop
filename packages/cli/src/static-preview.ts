...
export function iconFromPng(bytes: Uint8Array, size = ICON_SIZE): Uint8Array {
  ...
  const output =
    image.depth === 16 ? new Uint16Array(size * size * 4) : new Uint8Array(size * size * 4);
  ...
  const samples = (
    ...
    if (ratio < 1) {
      ...
      const lower = Math.floor(center),
        fraction = center - lower;
      return [
        [Math.max(0, Math.min(sourceSize - 1, lower)), 1 - fraction],
        [Math.max(0, Math.min(sourceSize - 1, lower + 1)), fraction],
      ];
    }
    const start = position * ratio,
      end = (position + 1) * ratio;
    const result: [number, number][] = [];
    for (let index = Math.floor(start); index < Math.ceil(end); index++) {
      result.push([
        Math.min(sourceSize - 1, index),
        Math.max(0, Math.min(end, index + 1) - Math.max(start, index)) / ratio,
      ]);
    }
    return result;
  };
        ...
        for (const [sx, wx] of columns[x]!) {
          ...
          const a =
            (image.channels === 2 || image.channels === 4
              ? image.data[offset + image.channels - 1]!
              : alpha) / alpha;
