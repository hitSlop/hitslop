...
export async function syncAgentSkills(
  options: SyncAgentSkillsOptions = {},
  ...
  try {
    ...
    if (installed) {
      ...
      try {
        if (options.missingOnly && (await info(cache)))
          await cp(cache, staging, { recursive: true });
        for (const name of options.missingOnly ? missing : managedSkillNames) {
          if (!(await stat(join(source, name, "SKILL.md"))).isFile())
            throw new Error(`Bundled skill is missing: ${name}`);
    ...
    for (const root of options.discoveryRoots ?? defaultSkillDiscoveryRoots()) {
      ...
      for (const name of managedSkillNames) {
        ...
        if (existing?.isSymbolicLink()) {
          ...
          if (!destination.startsWith(`${cache}${sep}`)) {
            conflicts.push(link);
            continue;
          }
          await rm(link);
...
async function lockSkills(cache: string) {
  ...
  if (process.platform !== "darwin")
    throw new Error(
      "Skill sync requires macOS or Linux. Install the bundled skills with your agent's skill installer on this platform.",
    );
  const O_EXLOCK = 0x20;
  for (let attempt = 0; ; attempt++) {
    try {
      return await open(
        `${cache}.lock`,
        constants.O_CREAT | constants.O_RDWR | constants.O_NONBLOCK | O_EXLOCK,
        0o600,
      );
    } catch (error) {
