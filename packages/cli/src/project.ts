...
export async function scaffold(destination: string, options: ScaffoldOptions): Promise<void> {
    ...
    JSON.stringify(
      {
        ...
        dependencies: {
          ...
          "@hitslop/svelte": "^0.3.0",
          "bits-ui": "^2.19.0",
          svelte: "^5.57.0",
          typebox: "^1.3.32",
        },
        devDependencies: {
          "@hitslop/cli": `^${cliPackage.version}`,
  ...
  await writeFile(
    appPath,
    (await readFile(appPath, "utf8")).replace("__SLOP_TITLE_LITERAL__", JSON.stringify(title)),
  );
}

export async function buildSlop(
  root: string,
): Promise<{ directory: string; manifest: SlopManifest }> {
  ...
  try {
    ...
    await viteBuild({
      root,
      plugins: [hitSlopBuildPlugin({ hasTheme })],
      build: { outDir: viteOut, emptyOutDir: true },
    });
    const html = join(viteOut, "index.html");
    if (!(await exists(html))) throw new Error("Vite did not produce index.html");
    ...
    if (dataSchema)
      await writeFile(join(output, "data.schema.json"), `${JSON.stringify(dataSchema, null, 2)}\n`);
    if (dataSchema) {
      const initialPath = join(root, "initial.ts");
      if (!(await exists(initialPath))) throw new Error("S.Document projects require initial.ts");
      const initial = validateDocument(
        applicationSchema(dataSchema),
        await evaluateModule(initialPath, "initial"),
      );
      await mkdir(join(output, "assets"), { recursive: true });
      await writeFile(join(output, "assets/initial.json"), `${JSON.stringify(initial)}\n`);
    }
...
export async function validateAuthoringProject(root: string): Promise<SlopManifest> {
  ...
  if (await exists(join(root, "schema.ts"))) {
    const packaged = await loadDataSchema(join(root, "schema.ts"));
    validateDocument(
      applicationSchema(packaged),
      await evaluateModule(join(root, "initial.ts"), "initial"),
    );
  }
  await readDocumentGuideSource(root);
  return manifest;
...
export async function packSlop(directory: string): Promise<{ bytes: Uint8Array; sha256: string }> {
  ...
  for (const path of await walk(directory))
    files[path] = [
      new Uint8Array(await Bun.file(join(directory, path)).arrayBuffer()),
      { mtime: epoch, level: 9 },
    ];
...
export async function writePackedSlop(path: string): Promise<string> {
  ...
  const runtime =
    (await exists(join(root, "app.html"))) && (await exists(join(root, "manifest.json")));
...
async function copyRuntimeTree(source: string, destination: string): Promise<void> {
  ...
  if (info.isDirectory()) {
    ...
    for (const entry of await readdir(source))
      await copyRuntimeTree(join(source, entry), join(destination, entry));
    return;
  }
