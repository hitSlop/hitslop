...
export async function validateDocumentSkill(root: string): Promise<void> {
  ...
  await requireOnly(
    references,
    new Set(["app-guide.md"]),
    ".agents/skills/hitslop-document/references",
  );
...
async function requireOnly(
  ...
  for (const entry of await readdir(directory, { withFileTypes: true })) {
    ...
    if (
      ...
      (optionalReferences && entry.name === "references" && !entry.isDirectory())
    ) {
