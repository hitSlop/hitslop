...
export function themeContract(source: string | undefined): Set<string> | undefined {
...
export async function readThemeCSS(root: string): Promise<string | undefined> {
  ...
  if (await exists(definition)) {
    ...
    if (typeof css !== "string") throw new Error("theme.ts must default-export defineTheme(...)");
...
export async function validateThemeOverride(
  root: string,
...
export function validateThemeReferences(
  ...
  const referenced = new Set(
    [...cssOrHTML.matchAll(/var\(\s*(--slop-[a-z0-9-]+)/gi)].map((match) => match[1]!),
  );
  if (!referenced.size) return;
  if (!contract)
    throw new Error(
      `${label} references public --slop-* variables but assets/theme.css is missing.`,
    );
  for (const property of referenced)
    if (!contract.has(property))
      throw new Error(
        `${label} references ${property}, but assets/theme.css does not provide a default.`,
      );
}
...
export function parseTheme(source: string, label: string): Set<string> {
  ...
  try {
    ...
    throw new Error(
      `${label} is not valid CSS: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
