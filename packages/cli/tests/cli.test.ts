...
async function runInit(arguments_: string[], skillsRoot?: string) {
  ...
  const child = Bun.spawn(
    ["bun", resolve(import.meta.dir, "../src/cli.ts"), "init", ...arguments_],
    {
      stdin: "ignore",
      stdout: "pipe",
      stderr: "pipe",
...
describe("slop init author metadata", () => {
  test("requires an author name for non-interactive init", async () => {
    const parent = await mkdtemp(join(tmpdir(), "hitslop-init-"));
    roots.push(parent);
  ...
  test("writes validated author attribution into the scaffold manifest", async () => {
    ...
    const result = await runInit([
      ...
      "  Jordan Singer  ",
      "--author-url",
      "https://example.com/jordan",
    ]);
    expect(result.exitCode).toBe(0);
    expect(JSON.parse(await readFile(join(destination, "manifest.json"), "utf8")).author).toEqual({
      name: "Jordan Singer",
...
test("skill installation failure does not prevent project creation", async () => {
  ...
  expect(JSON.parse(await readFile(join(destination, "manifest.json"), "utf8")).author.name).toBe(
    "Test Author",
  );
});
