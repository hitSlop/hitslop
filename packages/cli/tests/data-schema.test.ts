...
test("S.Document schemas are packaged as disk envelopes", async () => {
  const { envelopeSchema } = await import("@hitslop/schema/document");
  const schema = await loadDataSchema(
    new URL("../../../examples/slops/quick-checklist/schema.ts", import.meta.url).pathname,
  );
  expect(schema).toEqual(dataSchemaFromJSON(envelopeSchema(checklistSchema)));
  expect(schema.type).toBe("object");
  expect(schema.required).toEqual(["$slop", "data"]);
  expect(JSON.stringify(schema)).not.toContain("~standard");
});

test("plain schemas are rejected instead of creating unusable packages", async () => {
  const { mkdtemp, writeFile } = await import("node:fs/promises");
  const { tmpdir } = await import("node:os");
  const { join } = await import("node:path");
  const root = await mkdtemp(join(tmpdir(), "hitslop-json-schema-"));
  temporary.push(root);
  const path = join(root, "schema.ts");
  await writeFile(
    path,
    'export default { type: "object", properties: { title: { type: "string" } }, required: ["title"] };',
  );
  await expect(loadDataSchema(path)).rejects.toThrow("S.Document");
});
...
test("packaging reevaluates transitive imports and recovers after invalid edits", async () => {
  ...
  const schema = join(root, "schema.ts"),
    field = join(root, "field.ts");
  await writeFile(
    schema,
    'import field from "./field"; export default { type: "object", "x-hitslop": { version: 1, container: "map" }, properties: { field }, required: ["field"] };',
  );
  await writeFile(field, 'export default { type: "string" };');
  expect(((await loadDataSchema(schema)).properties as any).data.properties).toEqual({
    field: { type: "string" },
  });
...
test("distributed CLI resolves its sibling loader without source files", async () => {
  ...
  const build = await Bun.build({
    entrypoints: [
      ...
      join(cliRoot, "src/module-loader.ts"),
      join(cliRoot, "src/preview-authority.ts"),
    ],
    target: "bun",
    format: "esm",
    packages: "external",
  ...
  expect((await readdir(output)).sort()).toEqual([
    "cli.js",
    "module-loader.js",
    "preview-authority.js",
  ]);
  const { root } = await fixture("");
  await writeFile(
    join(root, "schema.ts"),
    'export default { type: "object", "x-hitslop": { version: 1, container: "map" }, properties: {} };',
  );
  await writeFile(
    ...
    JSON.stringify({
      ...
      slug: "loader-test",
      title: "Loader test",
      description: "A schema loading fixture",
      categories: ["utilities"],
      presentation: { width: 480, height: 620, shape: "rounded" },
    }),
  );
  await writeFile(join(root, "initial.ts"), "export default {};");
