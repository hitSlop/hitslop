...
async function fixture(): Promise<{ project: string; templates: string; preview: string }> {
  const root = await mkdtemp(join(tmpdir(), "hitslop-install-"));
  roots.push(root);
...
describe("installTemplate", () => {
  test("installs one store-free slop with static Quick Look images", async () => {
    ...
    expect(await readFile(join(result.directory, "QuickLook", "Preview.png"))).toEqual(
      await readFile(item.preview),
    );
    expect([
      decode(await readFile(join(result.directory, "QuickLook", "Icon.png"))).width,
      decode(await readFile(join(result.directory, "QuickLook", "Icon.png"))).height,
    ]).toEqual([512, 512]);
  ...
  test("asks before replacing and preserves an install when declined", async () => {
    ...
    expect(await readFile(join(item.templates, "tiny-counter.slop", "app.html"), "utf8")).toContain(
      "Hello",
    );
  });
  ...
  test("capture failure preserves the existing master", async () => {
    const item = await fixture();
    const installed = await installTemplate(item.project, {
      templatesRoot: item.templates,
      preview: item.preview,
      ...skipNativeIcon,
    });
  ...
  test("force replaces an existing install", async () => {
    ...
    const replaced = await installTemplate(item.project, {
      templatesRoot: item.templates,
      preview: item.preview,
      ...skipNativeIcon,
      force: true,
  ...
  test("captures a preview when no override is supplied", async () => {
    ...
    const result = await installTemplate(item.project, {
      templatesRoot: item.templates,
      ...skipNativeIcon,
      capturePreview: async (packageDirectory: string, output: string) => {
        captured = true;
  ...
  test("uses an author-supplied static icon", async () => {
    ...
    await writeFile(
      ...
      encode({
        ...
        height: 512,
        channels: 4,
        data: new Uint8Array(512 * 512 * 4).fill(127),
      }),
    );
    const result = await installTemplate(item.project, {
      templatesRoot: item.templates,
    ...
    expect(await readFile(join(result.directory, "QuickLook", "Icon.png"))).toEqual(
      await readFile(icon),
    );
  });
  ...
  test("uses an authored icon and removes stores initialized during its capture", async () => {
    ...
    const icon = Buffer.from(
      encode({
        ...
        height: 512,
        channels: 4,
        data: new Uint8Array(512 * 512 * 4).fill(91),
      }),
    );
    const result = await installTemplate(item.project, {
      templatesRoot: item.templates,
  ...
  test("requires two independently valid static preview files", async () => {
    ...
    await writeFile(
      join(result.directory, "QuickLook", "Icon.png"),
      encode({
        width: 512,
        height: 512,
        channels: 4,
    ...
    await expect(
      validateTemplatePackage(result.directory, { requirePreview: true }),
...
test("default icons fit previews inside a 512 pixel square", () => {
  const source = encode({
    ...
    height: 256,
    channels: 4,
    data: new Uint8Array(1024 * 256 * 4).fill(200),
  });
...
test("fallback icon downsampling averages fine detail and respects transparent edges", () => {
  ...
  const icon = decode(
    iconFromPng(encode({ width: 1024, height: 1024, channels: 4, data: checker })),
  );
