...
async function fixture(width = 240, height = 180, channels = 4): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), "hitslop-skin-"));
  roots.push(root);
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      slug: "skin-test",
      title: "Skin Test",
      description: "A PNG skin validation fixture.",
  ...
  await writeFile(
    join(root, "assets/skin.png"),
    encode({ width, height, channels, data: new Uint8Array(width * height * channels).fill(255) }),
  );
  return root;
}

describe("skin validation", () => {
  test("accepts exact RGBA dimensions", async () =>
    await expect(loadManifest(await fixture())).resolves.toMatchObject({
      presentation: { skin: "assets/skin.png" },
    }));
  test("rejects different dimensions", async () =>
    await expect(loadManifest(await fixture(241, 180))).rejects.toThrow("exactly 240x180"));
  test("rejects PNGs without alpha", async () =>
    await expect(loadManifest(await fixture(240, 180, 3))).rejects.toThrow("RGBA PNG"));
});
...
describe("immutable artifacts", () => {
  test("failed rebuild leaves the previous artifact intact", async () => {
    ...
    if (await Bun.file(join(root, "schema.ts")).exists())
      await writeFile(join(root, "initial.ts"), "export default { count: 0 };");
  ...
  test("preserves emitted worker, dynamic module, font, and wasm assets", async () => {
    ...
    await writeFile(
      ...
      '<script type="module" src="/main.js"></script><link rel="stylesheet" href="/src.css">',
    );
    await writeFile(
      join(root, "main.js"),
      'new Worker(new URL("./worker.js", import.meta.url)); import("./lazy.js").then(console.log); import wasm from "./module.wasm?url"; console.log(wasm);',
    );
    ...
    for (const prefix of ["worker-", "lazy-", "module-", "font-"])
      expect(assets.some((name) => name.startsWith(prefix))).toBe(true);
  ...
  test("inlines authored CSS into app.html", async () => {
    ...
    await writeFile(
      join(root, "index.html"),
      '<link rel="stylesheet" href="/src.css"><main>Skin</main>',
    );
  ...
  test("builds without stores or a runtime stylesheet", async () => {
    ...
    await mkdir(join(root, "stores"));
    await writeFile(join(root, "stores/data.json"), "{}\n");
    if (await Bun.file(join(root, "schema.ts")).exists())
      await writeFile(join(root, "initial.ts"), "export default { count: 0 };");
    const built = await buildSlop(root);
  ...
  test("builds immutable theme and schema metadata without document stores", async () => {
    ...
    await writeFile(
      join(root, "index.html"),
      '<main style="color:var(--slop-accent)">Themed</main>',
    );
    await writeFile(
      join(root, "schema.ts"),
    ...
    expect(await readFile(join(built.directory, "assets", "theme.css"), "utf8")).toContain(
      "--slop-accent",
    );
    expect(
      JSON.parse(await readFile(join(built.directory, "data.schema.json"), "utf8")),
    ).toMatchObject({ properties: { data: { properties: { count: { type: "number" } } } } });
    ...
    expect(
      await readFile(join(built.directory, ".agents/skills/hitslop-document/SKILL.md"), "utf8"),
    ).toBe(documentSkillContent);
  ...
  test("validates document data, theme overrides, and the canonical skill", async () => {
    ...
    await writeFile(
      ...
      ":root { --slop-accent: hotpink; }\n",
    );
    await expect(validateRuntimePackage(built.directory)).resolves.toMatchObject({
      slug: "skin-test",
    });
    await writeFile(
      join(built.directory, "stores/data.json"),
  ...
  test("requires every public theme reference to have an immutable default", async () => {
    ...
    await writeFile(
      join(root, "index.html"),
      '<main style="color:var(--slop-missing)">Themed</main>',
    );
    await writeFile(
      join(root, "assets", "theme.css"),
      ":root { --slop-accent: var(--slop-missing); }\n",
    );
  ...
  test("rejects invalid schema semantics and non-UTF-8 themes", async () => {
    ...
    const built = await buildSlop(root);
    await writeFile(
      join(built.directory, "data.schema.json"),
  ...
  test("copies one bounded publisher document guide", async () => {
    ...
    expect(
      await readFile(
        join(built.directory, ".agents/skills/hitslop-document/references/app-guide.md"),
        "utf8",
      ),
    ).toContain("Keep count");
...
describe("authoring scaffold", () => {
  test("includes agent notes, complete scripts, Bits UI, and export-aware starter", async () => {
    const root = await mkdtemp(join(tmpdir(), "hitslop-scaffold-"));
    roots.push(root);
    await scaffold(root, {
      title: "Tiny Tally",
      description: "Counts a tiny thing.",
      categories: ["utilities"],
      author: { name: "Jordan Singer", url: "https://example.com/jordan" },
    });
    ...
    expect(packageJSON.dependencies["@hitslop/schema"]).toBe("^0.3.0");
    ...
    expect(packageJSON.dependencies.typebox).toBe("^1.3.32");
    ...
    expect(JSON.parse(await readFile(join(root, "manifest.json"), "utf8")).author).toEqual({
      name: "Jordan Singer",
      url: "https://example.com/jordan",
    });
    await expect(
      readFile(join(root, ".agents/skills/hitslop-authoring/SKILL.md")),
    ).rejects.toThrow();
...
test("starter schema checks without mutating data or stripping unknown fields", async () => {
  const { validateDocument } = await import("@hitslop/schema/document");
  const schema = (await import("../templates/svelte/schema.ts")).default;
  const value = { count: 7, future: { field: true } };
  const before = structuredClone(value);
  expect(validateDocument(schema, value)).toEqual(before);
  for (const invalid of [{}, { count: "7" }, { count: 1.5 }, { count: Infinity }]) {
    expect(() => validateDocument(schema, invalid)).toThrow();
  }
});
