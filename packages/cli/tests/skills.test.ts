...
describe("syncAgentSkills", () => {
  test("writes the machine cache and discovery links without touching other skills", async () => {
    const root = await mkdtemp(join(tmpdir(), "hitslop-skills-"));
    roots.push(root);
    ...
    expect(await readFile(join(cacheRoot, "hitslop-document", "SKILL.md"), "utf8")).toBe(
      documentSkillContent,
    );
    expect(await readFile(join(agents, "other-skill", "SKILL.md"), "utf8")).toBe("keep me\n");

    const second = await syncAgentSkills({
      cacheRoot,
      discoveryRoots: [agents, claude],
      missingOnly: true,
    });
  ...
  test("preserves existing directories and unrelated links, repairing only managed links", async () => {
    ...
    expect(result.conflicts).toEqual([
      join(agents, "hitslop-authoring"),
      join(agents, "hitslop-design"),
    ]);
    ...
    expect(await readlink(join(agents, "hitslop-document"))).toBe(
      join(cacheRoot, "hitslop-document"),
    );
  });
});
...
test("packaged CLI skills stay in sync with canonical guidance", async () => {
    ...
    const walk = async (relative: string): Promise<void> => {
      for (const entry of await readdir(join(source, relative), { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) await walk(path);
          ...
          expect(await readFile(join(bundled, path), "utf8")).toBe(
  ...
  expect(await readFile(join(bundled, "hitslop-document", "SKILL.md"), "utf8")).toBe(
    documentSkillContent,
  );
});
...
test("bootstrap fills missing skills while explicit sync replaces the bundle", async () => {
  ...
  expect(await readFile(join(options.cacheRoot, "hitslop-design", "SKILL.md"), "utf8")).toContain(
    "name: hitslop-design",
  );
  await syncAgentSkills(options);
...
test("failed staging preserves the complete previous bundle", async () => {
  ...
  for (const name of managedSkillNames) {
    expect(await readFile(join(options.cacheRoot, name, "SKILL.md"), "utf8")).toContain(
      `name: ${name}`,
    );
  }
});
...
test("activation failure rolls back and a later sync can retry", async () => {
  ...
  const failingRename = spyOn(fs, "rename").mockImplementation(async (from, to) => {
    if (String(from).includes(".staging-") && to === options.cacheRoot)
      throw new Error("activation failed");
    return original(from, to);
  });
  try {
    await expect(syncAgentSkills(options)).rejects.toThrow("activation failed");
  } finally {
...
test("bootstrap recovers the previous tree after an interrupted activation", async () => {
  ...
  await syncAgentSkills({ ...options, missingOnly: true });
  expect(await readFile(join(options.cacheRoot, "hitslop-authoring", "SKILL.md"), "utf8")).toBe(
    "previous guidance",
  );
});
...
test("competing processes install complete bundles and release their locks", async () => {
  ...
  const children = bundles.map((bundledRoot) =>
    Bun.spawn(
      [
        process.execPath,
        "-e",
  ...
  const contents = await Promise.all(
    managedSkillNames.map((name) => readFile(join(options.cacheRoot, name, "SKILL.md"), "utf8")),
  );
