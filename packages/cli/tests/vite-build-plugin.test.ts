...
test("build plugin inlines JavaScript safely", () => {
  ...
  expect(inlineScript(html, "index.abcd.js", 'console.log("</script>")')).toBe(
    '<script type="module" crossorigin>console.log("\\x3C/script>")</script>',
  );
});
...
test("build plugin inlines generated structural CSS", () => {
  ...
  expect(inlineStyle(html, "index.abcd.css", "body { color: red; }")).toContain(
...
test("preserves unlinked chunks and escapes CSS raw-text terminators", () => {
  ...
  expect(
    inlineStyle('<link href="style.css">', "style.css", 'p:before{content:"</style>"}'),
  ).toContain("<\\/style>");
});
