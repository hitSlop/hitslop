...
test("template path policy rejects nested source, traversal, and mutable stores", () => {
  for (const path of [
    ...
    expect(() => validateTemplatePath(path)).toThrow();
  for (const path of [
    "manifest.json",
    "app.html",
    "assets/",
    ...
    expect(() => validateTemplatePath(path)).not.toThrow();
...
test("PNG header guard rejects allocation bombs and wrong icon dimensions before decoding", () => {
  ...
  expect(checkPngDimensions(bytes, "Icon", { width: 512, height: 512 })).toEqual({
    width: 512,
    height: 512,
  });
  expect(() => checkPngDimensions(bytes, "Skin", { width: 300, height: 200 })).toThrow(
    "exactly 300x200",
  );
