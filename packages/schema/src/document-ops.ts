...
import { documentMeta, recordValueSchema, prepareDocument } from "./document.js";
import {
  fail,
  OpsSchema,
  validate,
...
import type { SchemaNode } from "./document-paths.js";
...
export function applyOps(
  ...
  before: JSONValue,
  ops: readonly Op[],
  timing?: { applyMs: number; validateMs: number },
  ...
  try {
    prepared = prepareDocument(schema, candidate);
  } catch (error) {
    fail("validation", error instanceof Error ? error.message : String(error));
  }
  if (timing) {
    timing.applyMs = applied - start;
    timing.validateMs = performance.now() - applied;
  }
  return prepared!;
}
