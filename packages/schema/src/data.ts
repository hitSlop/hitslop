...
/** Generated by `@hitslop/schema/document`. Authors never write this keyword. */
export { hitslopSyncKeyword } from "./keywords.js";
import { hitslopSyncKeyword } from "./keywords.js";

/** Interpret the metaschema without dynamic code generation (Workers disallow eval). */
export function checkDataSchema(schema: Record<string, unknown>): void {
  ...
  if (!Check(Meta, Meta[dataSchemaDialect], schema))
    throw new Error("Invalid JSON Schema draft 2020-12 document");
}
...
export function compileDataSchema(schema: Record<string, unknown>) {
  ...
  addFormats(ajv);
  ajv.addKeyword({
    keyword: hitslopSyncKeyword,
    metaSchema: { type: "object", additionalProperties: true },
  });
  ajv.compile(schema);
...
export function dataSchemaFromJSON(value: unknown): Record<string, unknown> {
  ...
  assertJSON(value);
  const schema = { $schema: dataSchemaDialect, ...JSON.parse(JSON.stringify(value)) } as Record<
    string,
    unknown
  >;
